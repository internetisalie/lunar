 ...
 */
class LuaRocksPopularService(
    private val fetcher: PopularHtmlFetcher = defaultFetcher,
) {
    ...
    fun fetch(nowMs: Long = System.currentTimeMillis()): List<PopularEntry> {
        ...
        val entries =
            runCatching { PopularListParser.parse(fetcher.fetch(STATS_THIS_WEEK).orEmpty()) }
                .getOrElse { emptyList() }
