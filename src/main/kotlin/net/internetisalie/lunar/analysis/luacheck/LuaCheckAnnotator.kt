...
class LuaCheckAnnotator : ExternalAnnotator<LuaCheckAnnotator.Info, LuaCheckOutcome>() {
    ...
    override fun apply(
        file: PsiFile,
        annotationResult: LuaCheckOutcome,
        holder: AnnotationHolder,
    ) {
        val info = collectInformation(file) ?: return
        when (annotationResult) {
            is LuaCheckOutcome.Problems ->
                annotationResult.problems
                    .distinctBy { it.lineStart to it.message }
    ...
    private fun applyProblem(
        ...
        holder
            .newAnnotation(HighlightSeverity.WARNING, problem.message ?: "Unspecified problem")
            .range(range)
            .create()
    }
    ...
    private fun applyFailure(
        failure: LuaCheckOutcome.Failure,
        info: Info,
        holder: AnnotationHolder,
    ) {
        ...
        holder
            .newAnnotation(HighlightSeverity.WARNING, failure.detail)
            .range(range)
            .create()
    }
