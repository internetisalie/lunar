...
object PlatformVersionRegistry {
        ...
        mapOf(
            LuaPlatform.STANDARD to
                listOf(
                    ...
                    // Lua 5.5 uses the lua54 std (5.5 support is future)
                    VersionEntry("5.5", "lua-5.5", luacheckStd = "lua54"),
                ),
            LuaPlatform.LUAJIT to
                listOf(
                    VersionEntry("2.0", "luajit-2.0", luacheckStd = "luajit"),
                    VersionEntry("2.1", "luajit-2.1", luacheckStd = "luajit"),
                ),
            LuaPlatform.REDIS to
                listOf(
     ...
     */
    fun defaultVersion(platform: LuaPlatform): VersionEntry? = registry[platform]?.firstOrNull()
    ...
    fun findVersion(
        ...
        versionLabel: String,
    ): VersionEntry? = registry[platform]?.find { it.label == versionLabel }
    ...
    fun resolveTarget(
        ...
        val version =
            findVersion(platform, versionLabel)
