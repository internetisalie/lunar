...
class PlatformLibraryProvider : AdditionalLibraryRootsProvider() {
    override fun getAdditionalProjectLibraries(project: Project): Collection<PlatformLibrary> {
        val level = LuaProjectSettings.instance.state.languageLevel
        val platformDirectory = LuaFileUtil.getPluginVirtualDirectoryChild("platform", "Lua${level.version}") ?: return emptyList()
        val dir = VfsUtil.findFileByIoFile(File(platformDirectory.canonicalPath!!), true) ?: return emptyList()

        dir.children.forEach {
            it.putUserData(LuaFileUtil.PREDEFINED_KEY, true)
        }
        return listOf(PlatformLibrary(level, dir))
    }
