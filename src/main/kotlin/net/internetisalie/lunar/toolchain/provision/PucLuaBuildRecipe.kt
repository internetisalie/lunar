...
object PucLuaBuildRecipe {
    ...
    private fun archiveSteps(
        rc: RecipeContext,
        sources: List<String>,
    ): List<BuildStep> {
        val libObjs =
            sources
                .map { it.removeSuffix(".c") + ".o" }
    ...
    private fun linkSteps(
        ...
        if (sources.contains("luac.c")) {
            ...
            steps +=
                BuildStep(listOf(rc.cc, "-o", "luac", "luac.o") + printObj + listOf(rc.libName) + rc.ldflags, rc.srcDir)
        }
    ...
    private fun installCopies(
        ...
        val hpp =
            if (rc.srcDir
                ...
                rc.srcDir.resolve("lua.hpp")
            } else {
                rc.srcDir.parent.resolve("etc/lua.hpp")
            }
    ...
    private fun cflags(version: String): List<String> {
        ...
        if (majorMinor(version) ==
            ...
            flags += listOf("-DLUA_USE_STRTODHEX", "-DLUA_USE_AFORMAT", "-DLUA_USE_LONGLONG")
        }
    ...
    fun compatDefines(version: String): List<String> =
        when (majorMinor(version)) {
            "5.1" -> emptyList()
    ...
    private fun ldflags(os: LuaOs): List<String> =
        when (os) {
