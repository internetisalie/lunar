...
object LuaBlockStructure {
    ...
    private fun leafAt(
        file: PsiFile,
        offset: Int,
    ): PsiElement? = file.findElementAt(offset) ?: if (offset > 0) file.findElementAt(offset - 1) else null

    /** Whole statements of [block] fully inside the whitespace-trimmed selection; empty if any is split. §3.3. */
    fun statementsInRange(
        block: LuaBlock,
        startOffset: Int,
        endOffset: Int,
    ): List<LuaStatement> {
    ...
    fun caretAfterWrap(inserted: PsiElement): Int {
        ...
        return body
            ...
            ?.firstOrNull()
            ?.textRange
            ?.startOffset ?: inserted.textRange.startOffset
    }
    ...
    fun primaryBody(construct: PsiElement): LuaBlock? = (construct as? LuaBlockParent)?.getBlockList()?.firstOrNull()
...
data class LuaIfBranch(
    val condition: LuaExpr?,
    val body: LuaBlock,
