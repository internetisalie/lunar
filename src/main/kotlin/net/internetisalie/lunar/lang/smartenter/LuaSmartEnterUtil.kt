...
object LuaSmartEnterUtil {
    ...
    fun terminatorText(terminator: IElementType): String = if (terminator == LuaElementTypes.UNTIL) "until" else "end"

    /** True if [opener] already has its matching terminator token somewhere after it (idempotent guard). */
    fun alreadyTerminated(
        opener: PsiElement,
        terminator: IElementType,
    ): Boolean {
    ...
    private fun closerText(closer: IElementType): String =
        when (closer) {
    ...
    fun leafTokens(element: PsiElement): List<PsiElement> =
        PsiTreeUtil
            .collectElementsOfType(element, PsiElement::class.java)
