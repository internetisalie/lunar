...
class LuaNumeralAnnotator : Annotator {
    ...
    private fun annotateDecimalNumeral(
        text: String,
        element: PsiElement,
        holder: AnnotationHolder,
    ) {
        ...
        if (afterSign.isEmpty()) {
            holder
                .newAnnotation(HighlightSeverity.ERROR, "Malformed number: exponent has no digits")
                .range(element)
                .create()
        }
    }

    private fun annotateHexNumeral(
        text: String,
        element: PsiElement,
        holder: AnnotationHolder,
    ) {
        ...
        if (expIdx == -1) {
            if (afterPrefix.filter { it in "0123456789abcdefABCDEF." }.isEmpty()) {
                holder
                    .newAnnotation(HighlightSeverity.ERROR, "Malformed number: expected hexadecimal digit after '0x'")
                    .range(element)
                    .create()
            }
            return
        ...
        if (hexPart.isEmpty()) {
            holder
                .newAnnotation(
                    ...
                    "Malformed number: expected hexadecimal digit before exponent",
                ).range(element)
                .create()
        }
    ...
    private fun annotateSemanticKind(
        text: String,
        element: PsiElement,
        holder: AnnotationHolder,
    ) {
        val isFloat =
            text.contains('.') ||
                text.contains('e', ignoreCase = true) &&
...
class LuaAttribNameAnnotator : Annotator {
    ...
    private fun annotateLocalVarDecl(
        ...
        if (hasAttribute && target.exprList == null) {
            holder
                .newAnnotation(HighlightSeverity.ERROR, "Local variables with attributes must be initialized")
                .range(target)
                .create()
        }
    }
