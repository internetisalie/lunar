...
class RecordingTestEventsProcessor(
    ...
    override fun onTestStarted(testStartedEvent: TestStartedEvent) {
        events.add(
            "onTestStarted: name=${testStartedEvent.name}, id=${testStartedEvent.id}, parentId=${testStartedEvent.parentId}, url=${testStartedEvent.locationUrl}",
        )
    }

    override fun onTestFinished(testFinishedEvent: TestFinishedEvent) {
        events.add(
            "onTestFinished: name=${testFinishedEvent.name}, id=${testFinishedEvent.id}, duration=${testFinishedEvent.duration}",
        )
    }

    override fun onTestFailure(testFailedEvent: TestFailedEvent) {
        val expActStr =
            if (testFailedEvent.comparisonFailureExpectedText != null ||
                testFailedEvent.comparisonFailureActualText != null
            ) {
        ...
        events.add(
            "onTestFailure: name=${testFailedEvent.name}, id=${testFailedEvent.id}, msg=${testFailedEvent.localizedFailureMessage}, trace=${testFailedEvent.stacktrace}, isError=${testFailedEvent.isTestError}, duration=${testFailedEvent.durationMillis}$expActStr",
        )
    }

    override fun onTestIgnored(testIgnoredEvent: TestIgnoredEvent) {
        events.add(
            "onTestIgnored: name=${testIgnoredEvent.name}, id=${testIgnoredEvent.id}, comment=${testIgnoredEvent.ignoreComment}",
        )
    }
    ...
    override fun onSuiteStarted(suiteStartedEvent: TestSuiteStartedEvent) {
        events.add(
            "onSuiteStarted: name=${suiteStartedEvent.name}, id=${suiteStartedEvent.id}, parentId=${suiteStartedEvent.parentId}, url=${suiteStartedEvent.locationUrl}",
        )
    }
    ...
    override fun onUncapturedOutput(
        text: String,
        outputType: Key<*>,
    ) {
...
class LuaTestRunnerTest : BaseDocumentTest() {
    ...
    fun testLunityJsonLineParsing() {
        ...
        converter.process(
            "{\"event\":\"test_start\",\"name\":\"test_connect\",\"file\":\"db_test.lua\",\"line\":10}\n",
            ProcessOutputTypes.STDOUT,
        )
        converter.process(
            "{\"event\":\"test_pass\",\"name\":\"test_connect\",\"duration\":12}\n",
            ProcessOutputTypes.STDOUT,
        )
    ...
    fun testLunityFailedAndIgnoredTests() {
        ...
        converter.process(
            "{\"event\":\"test_start\",\"name\":\"test_fail\",\"file\":\"t.lua\",\"line\":5}\n",
            ProcessOutputTypes.STDOUT,
        )
        converter.process(
            "{\"event\":\"test_fail\",\"name\":\"test_fail\",\"message\":\"expected 4 but got 5\",\"trace\":\"traceback...\",\"duration\":5}\n",
            ProcessOutputTypes.STDOUT,
        )
        converter.process(
            "{\"event\":\"test_ignore\",\"name\":\"test_pending\",\"message\":\"not done yet\"}\n",
            ProcessOutputTypes.STDOUT,
        )
    ...
    fun testBustedMixedStreamRouting() {
        ...
        val jsonLine =
            "{\"successes\":[{\"name\":\"tests → foo\"," +
                "\"trace\":{\"source\":\"@t.lua\",\"currentline\":1},\"duration\":0}]}"
    ...
    fun testBustedPathPrependedFromEnvBuilder() {
        val toolsDir =
            com.intellij.openapi.util.io.FileUtil
                .createTempDirectory("lunar-tools", null)
        ...
        try {
            ...
            assertTrue(
                pathEnv.startsWith(toolsDir.absolutePath + File.pathSeparator) || pathEnv == toolsDir.absolutePath,
            )
        } finally {
            resetToolchain()
        }
    ...
    fun testLuaTestLocator() {
        ...
        com.intellij.openapi.application.runReadAction {
            ...
            val document =
                com.intellij.openapi.fileEditor.FileDocumentManager.getInstance().getDocument(
                    psiFile.virtualFile,
                )!!
        ...
        try {
            ...
            val runningModel =
                object : com.intellij.execution.testframework.TestFrameworkRunningModel {
                    ...
                    override fun getTreeBuilder(): AbstractTestTreeBuilderBase<*>? = null
            ...
            val mockConsoleView =
                object : com.intellij.openapi.ui.ComponentContainer {
    ...
    fun testBustedApostropheDoesNotSwallowJson() {
        ...
        assertTrue(
            processor.events.contains(
                "onTestStarted: name=it doesn't crash, id=test_2, parentId=suite_1, url=lua://t.lua:3",
            ),
