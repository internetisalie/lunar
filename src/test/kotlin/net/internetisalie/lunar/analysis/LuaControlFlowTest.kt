...
class LuaControlFlowTest : BasePlatformTestCase() {
    ...
    fun testSimpleBranchingReachability() {
        ...
        // Find the print call
        val printCall =
            PsiTreeUtil.findChildrenOfType(func, LuaFuncCall::class.java).firstOrNull {
                it.text.startsWith("print")
            }
    ...
    fun testReadWriteAccesses() {
        ...
        val rwInstructions =
            flow.instructions.filterIsInstance<LuaReadWriteInstruction>().filter {
                it.variableName == "a"
            }
    ...
    fun testNumericForLoopVariableWrite() {
        ...
        val writeInst =
            flow.instructions
                .filterIsInstance<LuaReadWriteInstruction>()
    ...
    fun testWhileLoopWithBreak() {
        ...
        val unreachableInst =
            flow.instructions.firstOrNull {
                it.element == unreachablePrint ||
                    it.element == unreachablePrint?.parent
            }
        ...
        val reachableInst =
            flow.instructions.firstOrNull {
                it.element == reachablePrint ||
                    it.element == reachablePrint?.parent
            }
    ...
    fun testNoSpuriousEdgeOutOfReturns() {
        ...
        val printCall =
            PsiTreeUtil
                .findChildrenOfType(func, LuaFuncCall::class.java)
    ...
    fun testGotoAndLabel() {
        ...
        val skippedInst =
            flow.instructions.firstOrNull {
                it.element == skippedPrint ||
                    it.element == skippedPrint?.parent
            }
        ...
        val reachedInst =
            flow.instructions.firstOrNull {
                it.element == reachedPrint ||
                    it.element == reachedPrint?.parent
            }
