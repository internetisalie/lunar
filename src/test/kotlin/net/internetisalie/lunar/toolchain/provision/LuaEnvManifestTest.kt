...
class LuaEnvManifestTest {
    ...
    @Test
    fun `write then read yields an equal object`(
        @TempDir dir: Path,
    ) {
    ...
    @Test
    fun `write uses the canonical file name`(
        @TempDir dir: Path,
    ) {
    ...
    @Test
    fun `well-formed but shape-invalid file reads as null`(
        @TempDir dir: Path,
    ) {
    ...
    @Test
    fun `skip rule skips when hash matches and binaries are executable`(
        @TempDir dir: Path,
    ) {
    ...
    @Test
    fun `skip rule does not skip on hash mismatch`(
        @TempDir dir: Path,
    ) {
        val component = executableComponent(dir, "abc")
    ...
    @Test
    fun `skip rule does not skip when a binary is missing`(
        @TempDir dir: Path,
    ) {
        val component =
            LuaManifestComponent(
