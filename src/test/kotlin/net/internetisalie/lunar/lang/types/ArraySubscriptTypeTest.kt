...
class ArraySubscriptTypeTest : IndexedBasePlatformTestCase() {
    ...
    fun testArraySubscriptInfersElementType_TC_IDX_1() {
        ...
        val subscript =
            PsiTreeUtil
                .findChildrenOfType(file, LuaIndexExpr::class.java)
                .first { it.expr != null }
    ...
    fun testNonArrayBracketStaysUndefined_TC_IDX_2() {
        ...
        val subscript =
            PsiTreeUtil
                ...
                .first { it.expr != null }
        assertEquals(
            "non-array bracket access must stay undefined",
            "undefined",
    ...
    fun testLengthOverArrayHasNoAssignabilityError() {
        ...
        assertEquals(
            "#arr over string[] must add no assignability error vs baseline: ${snapshot.getErrors().map {
                it.message
            }}",
            baselineErrors,
        ...
        assertTrue(
            "#arr over string[] must not emit a length-operand not-assignable error: ${snapshot.getErrors().map {
                it.message
            }}",
