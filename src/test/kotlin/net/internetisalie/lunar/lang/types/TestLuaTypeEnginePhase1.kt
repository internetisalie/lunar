...
class TestLuaTypeEnginePhase1 : BasePlatformTestCase() {
    ...
    fun testCircularReference() {
        ...
        val xVar =
            com.intellij.psi.util.PsiTreeUtil
                .findChildrenOfType(
                    file,
                    net.internetisalie.lunar.lang.psi.LuaLocalVarDecl::class.java,
                ).first()
                .attNameList
                .first()
                .nameRef
        ...
        assertTrue(
            "Circular reference should resolve to Undefined or Any",
    ...
    fun testMultipleAssignmentsChangingType() {
        ...
        assertTrue(
            "Variable with multiple assignments should be a union of those types, but got: ${type.displayName()}",
    ...
    fun testAnonymousFunctionReturnInference() {
        ...
        assertEquals(
            "Should infer number from anonymous function return",
            LuaGraphType.Number.displayName(),
            snapshot.getValueType(xVar).displayName(),
        )
    }
}
