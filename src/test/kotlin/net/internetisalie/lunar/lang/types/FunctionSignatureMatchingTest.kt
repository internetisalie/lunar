...
class FunctionSignatureMatchingTest : IndexedBasePlatformTestCase() {
    ...
    fun testArityOptionalOK() {
        ...
        val snapshot = LuaTypesSnapshot.forFile(file)
        assertTrue(
            "Should have no errors with optional parameter omitted, but got: ${snapshot.getErrors().map {
                it.message
            }}",
    ...
    fun testArgumentTypeMismatch() {
        ...
        assertTrue(
            "Error should mention number is not assignable to string, but was: ${errors.firstOrNull()?.message}",
    ...
    fun testContravarianceSuccess() {
        ...
        val snapshot = LuaTypesSnapshot.forFile(file)
        assertTrue(
            "Should have no errors for compatible function param, but got: ${snapshot.getErrors().map { it.message }}",
            snapshot.getErrors().isEmpty(),
        )
    }
