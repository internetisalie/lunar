...
- [ ] **Manual UI**: Verify name sanitization in the wizard prevents illegal rockspec filenames.

---

## ROCKS-02: Package Browser (#177)

### Phase 1: Search Service & Caching
...
### Phase 2: Split-View Tool Window
...
## ROCKS-03: Dependency Resolution (#178)
...
### Phase 2: Graph Logic & Conflict Engine
...
### Phase 3: Tree View & Visualization
...
## ROCKS-04: Task Execution & Run Configurations (#176)
...
### Verification: Unit & Manual Tests
- [ ] **Serialization**: Verify run configs are saved correctly to `.idea/runConfigurations`.
- [ ] **Environment**: Verify custom environment variables are passed to the `luarocks` process.

---

## ROCKS-DR: De-risking Tasks

### ROCKS-DR-01: Prototype resolution mechanism (#180)
...
### ROCKS-DR-02: Test init robustness (#181)
- [ ] **Existing Dir**: Run init on a directory with an existing `src/` folder. Verify it doesn't delete existing files.

### ROCKS-DR-03: Verify Lua JSON Bridge for Rockspec Parsing (#179)
- [ ] **Encoder Test**: Run `rockspec.lua` with the selected encoder and verify valid JSON output for all rockspec tables.
