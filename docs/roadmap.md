...
| BUG-445 | The documented `ktlintFormat` step formats the builder VM's copy and never reaches the repo | todo | M | — | found 2026-08-21 during BUG-422 verification. `gce-builder` rsync is one-directional, so `run "ktlintFormat ktlintCheck"` **cannot fail**: format rewrites the VM's copy, check passes against it, the committed tree keeps every violation. `ktlintCheck` alone is RED on main @ `9c78a037` (13 files from recent bug-fix commits). CI runs no ktlint at all. Fix the instruction, not just the debt | — |
