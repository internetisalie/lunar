...
| BUG-438 | The `@class` completion door misses the 100 ms time-to-first budget — 323 ms at 3 600 members | planned | M | — | **PLANNED 2026-08-20 — root cause grounded.** `LuaImplicitFields.collect` (`LuaImplicitFields.kt:75`) is what de-stubs: `PsiTreeUtil.findChildrenOfType(file, LuaAssignmentStatement)` loads the AST and walks every node, per declaring file, per cold materialization. Proved with `PsiFileImpl.isContentsLoaded` staged through `materializeClass` — `[false]` up to `afterAllHostedParts`, `[true]` after implicit fields. **This corrects the earlier bucketing**: the ~92 ms attributed to "parse" IS this call's de-stub, so it is ~63 % of the door, not two separate buckets. Strategy: guard the walk with the index (which cannot replace it — it stores no type, COMP-09 §4.13). **Measure the catalog FIRST**: love2d has 0 implicit assignments and openresty 78, so the win is shape-dependent and n=2 is not a basis | ✓ |
| BUG-440 | Quick Doc returns "No documentation found" for openresty members, while love2d works | planned | S | — | **PLANNED 2026-08-20 — it is NOT openresty, it is `---@field`.** Probed with the real catalog files: `ngx.say` and `love.getVersion` (both `function X.y()`) each yield 1 doc target with HTML; `ngx.status` AND `LoveConfig.identity` (both `---@field`) each yield **0**. A love2d field fails identically to an openresty one, so the library split this report was built on is a red herring — the checklist tested a FUNCTION on love2d and a FIELD on openresty. Root cause: `resolveDocumentationTarget:108-126` goes through `reference.resolve()`, and a `@field` has no declaration PSI (the AGENTS.md invariant). Fix: target the tag's own element via `LuaClassType.resolveMember`'s `sourceElement`, as `LuaCatsTypeNavigation` already does for a bare `---@class`. **One thing unresolved**: `ngx.say` failed LIVE but does not reproduce headlessly — verify-in-ide before closing | — |
