...
| BUG-439 | A global's members declared in a **sibling file** are never offered — love2d's whole submodule API is unreachable | todo | **M** | — | found by COMP-09's live IDE verification (2026-08-14) against real catalog libraries. `love.` offers its 40 own members and **none of the 19 submodules** (`love.graphics`, `love.audio`, …), each a `love.graphics = {}` in a sibling file; `love.graphics.` is empty too, so love2d's 100-function graphics API is unreachable from either direction. Same-file assignments enumerate fine (`ngx.` offers its `ngx.X =` constants), so the file boundary is the variable. **Regression vs pre-existing is UNSETTLED — measure it on a detached worktree at `fb79c038`/`87875c9f`, do not infer.** Design §4.5's first-declaring-file rule exists because DR-09 measured a flat union returning a superset, so a naive widening re-breaks COMP-09-06 | ✓ |
| COMP-09 | Member enumeration — "members of X" from an index, not a key scan or file walk; a latency gate and a work bound | **done** — all six phases complete (2026-08-12). **Phase 5** re-measured both doors and settled every deferral: the **completion door meets NFR-1 at 12.2 ms** (medians of five distinct wide receivers in five files) while the **`@class` door misses it at 269 ms direct / 323 ms through completion** — the residual design §1.6 predicted, deferred as **BUG-438**. DR-07/16/23/24 closed on numbers; DR-25 deferred as **BUG-437**; all four caches re-measured and all four kept. No production change in Phase 5. The live IDE checklist is **not** run — 15 of 18 scenarios are discharged by automated real-flow tests and the remaining 3 are a supervisor call | M | — | **Phase 4** closed BUG-426's known limitation: a `---@class`-declared `__add` now makes its instances arithmetic-capable, inherited metamethods included, with the offered member sets measured unmoved. **Re-planned 2026-08-12, Step 9 PASS at round 3.** Phases 0–1 stand: `LuaReceiverMemberIndex` is built, registered, tested and consumed by nothing. Phase 2's original site was executed, measured and aborted (dead for every receiver with members, and downstream of the cost), so **Phases 2–5 are re-cut** around a site **above** `LuaTypesSnapshot.forFile`, with Rule S as the explicit shadowing rule. Prototyped through `completeBasic()`: cold time-to-first **491 ms → 7.4 ms** (medians of 5), COMP-09-08 met at 8–13 ms against the 100 ms budget. Target is the cold library-graph build, **844–955 ms** for a 123 KiB / 3 600-member library (TYPE-11 DR-08). Surfaced BUG-430/431/432; DR-28 additionally measured that `redis.` gains ten `@field` constants on **five** shipped Redis/Valkey targets — a declared expectation (TC 7f/7f-bis), not a regression | ✓ |
