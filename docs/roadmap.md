...
| BUG-395 | Member completion after `.` is file-local, so library members never appear | todo | S | — | TARGET-08-04 (TC 6) | ✓ Found reviewing TARGET-08 Phase 4 (2026-08-03), after an earlier diagnosis wrongly folded this into BUG-394. The post-dot caret is served only by `LuaCompletionContributor.kt:249-283`, which reads `LuaTypesSnapshot.forFile(...).getValueType(...).getMembers()` and never consults `GlobalSymbolRankingService` — so **widening BUG-394's search scope will not fix this**. The member path is structurally file-local: `LuaTypesVisitor.visitNameRef:761` does `scope.lookup(text) ?: return`, and the only cross-file hooks are `resolveModule` for `require` and `seedAmbientGlobals` (which reads just `global.lua`). Consequence: `string.` does not complete either. This is what blocks TARGET-08's TC 6 |
