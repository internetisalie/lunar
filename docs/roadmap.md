...
| BUG-440 | Quick Doc returns "No documentation found" for openresty members, while love2d works | in_progress | S | — | **`---@field` half FIXED 2026-08-20; NOT closed — `verify-in-ide` owed.** Root cause was the declaration form, not the library: a love2d `@field` fails exactly as an openresty one does. `catsFieldDocumentationTarget` documents the tag via `LuaClassType.resolveMember`'s `sourceElement`; resolution is deliberately NOT made to succeed for a field. Suite green with corpus, 0 IMPROVED, mutation-proved 2/2. **Two things owed in one VNC session**: that the fix RENDERS (Quick Doc is user-visible and the skill requires VNC for it), and the `ngx.say` half — reported failing live, does not reproduce headlessly, and a headless green cannot settle it either way | — |
