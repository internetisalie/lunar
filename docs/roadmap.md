...
| TYPE-12 | A method call site mints its own member demand, and every one is wired into a clique | todo | M | — | — | needs planning: BUG-473's residual. Both BUG-473 phases removed the redundant re-derivation over the Θ(n²) cross-product (145 s → 3.6 s at 160 call sites) but not the cross-product itself; growth is still ×5.9 per doubling and the clique costs ~2.8 s at that size. Every cheap edge-pruning bound measured changes what the engine infers — the mildest deletes the `---@param` violation on every method call (now a red test), so this is a representation change, not an optimisation | ✓ |
| TYPE-13 | A structurally-resolved member carries no declaration | planned | M | — | **REFACT-09** | **DR-01 executed and inverted DR-03**: the structural route already resolves a plain table and a global table to the declaration's own `:m` node — no `className` needed, so this is not receiver classification. What is missing is that `tableToLuaType` builds every structural `LuaTypeMember` without a `sourceElement`. The trap DR-01 found: `setmetatable` OO resolves to the CALL SITE, so populating the field naively reintroduces DR-03's half-rename — hence a declaration mark carried on each member node at its mint site (`VariableNode.declaresMember`), never inferred from the node's PSI class. **DR-05** then measured the dropped declaration reachable from the winning node's `upSet`, so no merge is changed. Design+plan+risks written, phased per `implementation-plan.md` | ✓ |
| REFACT-09 | Colon-method rename — `function Obj:m()` and its call sites | todo | S | **TYPE-13** *(the declaration verdict)*; **TYPE-13-00-DR-02** *(the completeness verdict)*; REFACT-01 *(carved out of `-08`)* | closes `REFACT-01-08` | blocked: the refusal at `LuaRenameProcessor.kt:111` is correct today. DR-03 measured lifting it half-renaming three shapes — plain table, global table, `setmetatable` OO — each reporting SUCCESS. TYPE-13's DR-01 deleted the receiver-classification premise: the refusal's replacement is a *declaration* predicate — rename only where `LuaMemberDeclarations.declarationOf` returns one and `TYPE-13-00-DR-02`'s completeness rule allows it — keeping the existing refusal as its else-branch, plus the caret-on-`self` guard (~8 lines) | — |
| BUG-474 | A ratio gate amplifies machine speed instead of dividing it out, when one side is superlinear | todo | S | — | `LuaClassTagSnapshotPerformanceTest` passes at 87x on libvirt and fails at 340x on GCE, same commit, no edits; the premise holds only if both sides scale alike | ✓ |
...
| MAINT-39 | A LuaCATS-annotated corpus — the sweep cannot see the annotated path at all | todo | M | BUG-473 *(S6; the curve must be tolerable first)* | 0 of 734 corpus files carry any `---@` tag; corpus is pre-LuaCATS-era Lua, so the whole luacats subsystem and the annotated type engine are unswept — BUG-473 lived there undetected. Staged: DR-6 fixture, then S6, then real projects | ✓ |
