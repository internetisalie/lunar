...
| TYPE-11 | A library's type snapshot is discarded by any keystroke — `forFile` depends on project-wide `MODIFICATION_COUNT` | **in_progress** | M | — | **blocks COMP-09**; measured 9 ms vs 334 ms for the same 2-line file without/with a 123 KiB library (DR-20) | ✓ — planning complete 2026-08-11 after five Step 9 rounds. **Phase 1 done 2026-08-11**: `LuaTypeSourceRecorder` + `LuaLibraryProvenance` exist and are wired to nothing; TYPE-11-03 gated by `LuaLibraryProvenanceTest`, all six mutations re-earned against the production service. **Remediated after review FAIL**: both conservative markers dropped their mark on a null URL (a pin granted, design §3.1 step 4 clarified); the memoized dependency set was ungated (`NEVER_CHANGED` left all six green — measured); `LuaTypeSourceRecorderTest` added, 12 assertions each with an observed red |
| BUG-433 | `addMethodsOf` walks every global-declaration stub key with no `ProgressManager.checkCanceled()` | todo | S | — | found by TYPE-11 Phase 2 review (D2); contract §2 requires it of PSI lookup routines, and the whole `psi/types` package has zero. Fix standalone — a new cancellation point is a behaviour change | ✓ |
| BUG-434 | `LuaTypeReference`'s `by lazy` hides a consumed file from the recording frame, so a library snapshot is pinned while depending on a project file | todo | **M** | TYPE-11 *(Phase 3, the pin is live)* | found by TYPE-11 Phase 4 (DR-07), **reproduced end to end and attributed by mutation** — `arm2 pre-forced urls=[lib.lua] pinnable=true` vs `arm1 cold urls=[lib.lua, gadget.lua] pinnable=false`, and the pinned snapshot survives a project edit (`after=[spin] sameSnapshot=true` vs control `after=[spun] sameSnapshot=false`). A **sixth** under-recording channel; reproduction lives in `TypeElevenDr07LazyReferenceProbeTest` (printing only). Candidate fix is `by lazy` → `get()`, which makes every arm cold — but that is a hot-path change in a performance feature, so it needs DR-04, `provisioned=11 pinnable=11` and the corpus sweep re-measured | Serial: type-engine |
