...
| NAV-13 | Colon call site resolution — the reference direction REFACT-09 is blocked on | planned | M | TYPE-13 *(done, `0bccadae`)* | **REFACT-09** | Serial: rename machinery |
| REFACT-09 | Colon-method rename — `function Obj:m()` and its call sites | todo | S | **TYPE-13** *(done, `0bccadae`)*; REFACT-01 *(carved out of `-08`)* | closes `REFACT-01-08` | Serial: rename machinery |
...
| BUG-476 | Renaming a table leaves the receiver segment of its `function t:m()` declarations behind | todo | S | — | measured on the unmodified rename path at `128ba091`, in both the colon and the dotted spelling and from either caret; REFACT-09 Gap 2.1 records it and refuses to widen into it | ✓ |
...
| BUG-477 | Quick Documentation renders nothing for a LuaCATS-commented function | todo | S | — | — | ✓ |
| BUG-478 | Find Usages invoked at a declaration leaf does nothing, silently | todo | C | — | — | ✓ |
