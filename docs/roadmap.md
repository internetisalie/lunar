...
| BUG-439 | A global's members declared in a **sibling file** are never offered — love2d's whole submodule API is unreachable | todo | **M** | — | **two attempts reverted; the blocker is now ONE named test, and the remedy is known.** The fix is `membershipOver` unioning declaring files + `candidates` also consulting this index's key space (the union alone is a no-op: `LuaGlobalAssignmentIndex` lists only files that ASSIGN the global). Both BUG-439 fixtures then pass. Of the six failures, five are consequential — incl. `testCompletionDoorReadsExactlyOneFile`, which states this very defect as intent and should be rewritten. **The sixth is real**: `testAFileLocalReceiverIsNotASelectableDeclaringFile` — a `local wx = {}` is absent from `LuaGlobalAssignmentIndex` (the rule that saved the old selection) but PRESENT in this index's key space, so widening `candidates` re-admits it and reproduces DR-09's contamination exactly. **Remedy: a `LOCAL_BINDING` sentinel** beside the existing `OPAQUE_BINDING` one, emitted from `forEachBareBinding`, with `candidates` filtering on it — index-format work (getVersion bump) + COMP-09-08 latency re-run. **Regression-vs-pre-existing: ANSWERED, pre-existing** | ✓ |
