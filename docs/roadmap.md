...
| BUG-447 | **Add to Watches** is a silent no-op for every debugger variable | todo | S | — | carved out of BUG-414 on 2026-08-22, which had bundled it as a dead-field cleanup. Nothing overrides `getEvaluationExpression`, and `DebuggerUIUtil.addToWatches` drops a null expression with no user-visible signal — so the action is enabled and does nothing, for *every* variable rather than the nested table children the review named. The two fields the cleanup would have deleted are the surviving half of the feature, not leftovers. **Settle it live before writing code** | ✓ |
