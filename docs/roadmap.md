...
| BUG-398 | A required module's exported type loses its `@class` parent and leaks the file's locals | todo | C | — | — | ✓ Found while testing BUG-395 (2026-08-04). Given luassert's real shape — `---@class luassert : luassert.internal` on a `local`, `return luassert` — a consumer's `assert.` (via `assert = require("luassert")`) completes `[unregister, internal, luassert]`: the class's **own** member is right, but the `luassert.internal` parent contributes nothing and two file-scope locals surface *as members*. Upstream of BUG-395's fix, in how `LuaTypeManagerImpl.getModuleType` materializes a module's exported type; independent of the caret being a global. Practical effect on TARGET-08: a definition library's inherited members do not complete, so `assert.` is thin rather than empty |
