...
| MAINT-33 | Corpus sweep — regression ratchet over pinned real-world Lua projects | **done** | S | — | — | ✓ 2026-08-03 → `docs/features/maint/33-corpus-sweep/` (all 6 phases). Pinned checkouts (luacheck v1.2.0, luarocks v3.12.2, ZeroBrane 2.01) fetched out-of-repo to `test/corpus/`, swept headlessly for parse errors / unresolved `require`s / inspection hits / per-symbol breakdown / ballast, gated against committed baselines. Opt-in via `-PwithCorpus`; builder-only, never CI. **Found BUG-389, BUG-390, BUG-391, BUG-392 and BUG-393** — four fixed. Verification checklist run 2026-08-03: 11 pass, 1 fail (Scenario 4.1 — the ballast `claimed` flag is all-or-nothing, advisory/ungated, recorded as the DR-05 residual). KOReader admitted, measured and parked: fits on disk (12 MB) but sweeps in 10.2 min, taking the corpus past its 10-minute ceiling. Complements the manual `vnc-multi-project-smoke-tests.md` |
