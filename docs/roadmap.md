...
| TYPE-11 | A library's type snapshot is discarded by any keystroke — `forFile` depends on project-wide `MODIFICATION_COUNT` | **in_progress** | M | — | **blocks COMP-09**; measured 9 ms vs 334 ms for the same 2-line file without/with a 123 KiB library (DR-20) | ✓ — planning complete 2026-08-11 after five Step 9 rounds. **Phase 1 done 2026-08-11**: `LuaTypeSourceRecorder` + `LuaLibraryProvenance` exist and are wired to nothing; TYPE-11-03 gated by `LuaLibraryProvenanceTest`, all six mutations re-earned against the production service. **Remediated after review FAIL**: both conservative markers dropped their mark on a null URL (a pin granted, design §3.1 step 4 clarified); the memoized dependency set was ungated (`NEVER_CHANGED` left all six green — measured); `LuaTypeSourceRecorderTest` added, 12 assertions each with an observed red. **Phases 2, 3, 4 and 4b done 2026-08-11/12** — the pin is live and conditional; **all six** under-recording channels closed, the sixth being BUG-434 (a memoized `LuaTypeReference` answer now carries its `SourceFrame` and replays it on every read), at zero cost: `provisioned=11 pinnable=11`, routine 2630/0/1, corpus 2638/0/1, DR-04 arm B inside the harness noise band. Only Phase 5 (DR-08, the residual arm-B cost, `Could`) is open |
