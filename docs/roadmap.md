...
| REFACT-09 | Colon-method rename — `function Obj:m()` and its call sites | todo | S | **TYPE-13** *(done, `0bccadae`)*; REFACT-01 *(carved out of `-08`)* | closes `REFACT-01-08` | planned artifacts written; `REFACT-09-00-DR-02` executed. **The completeness verdict is not `declarationOf`'s.** DR-02 measured `ReferencesSearch` returning **0** call sites for a colon declaration in every receiver shape, the `---@class` one included, and `declarationOf` returning null for an alias, a parameter receiver, every `self:` call, a `require`d module and a cross-file global — i.e. null exactly where completeness is at risk. So the predicate decides containment **syntactically** (a unique file-local receiver binding whose value never leaves the file, over a whitelist closed on the `var` grammar's index and call steps) and uses `declarationOf` only to separate same-named members of different receivers. Measured end to end on a prototype: a plain local table, its `self:` calls and its annotated form rename; global, module, alias, parameter, factory, `setmetatable`, chained and mixed dot/colon shapes each refuse with a message naming the clause. Full suite 2 979 tests / 2 failures, both `LuaRenameTest` message assertions that Phase 4 rewrites | — |
