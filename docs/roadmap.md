...
| COMP-09 | Member enumeration — "members of X" from an index, not a key scan or file walk; a latency gate and a work bound | **ready** | M | — | **Unblocked 2026-08-12** — TYPE-11 is done, so the 334 ms recurring cost that made Phase 2's change site nearly dead is gone. Index built, tested and registered at Phase 1; **Phases 2–4 must be re-planned, not resumed** — the site was measured downstream of the cost. Surfaced BUG-430/431/432. **Target now measured (TYPE-11 DR-08, 2026-08-12):** the cold library-graph build it exists to remove is **844–955 ms** for a 123 KiB / 3 600-member library (~690 ms AST traversal + ~170 ms `checkTypes`), 58× the recurring residual DR-08 declined to chase | ✓ |
