...
| BUG-441 | Unknowns are omitted rather than represented — an unknown write vanishes instead of widening | todo | S | — | carved out of BUG-419 (closed 2026-08-14) so a measured-zero item stops holding a report open. Criterion 1 of BUG-419's emission rule, still unimplemented: a node-less unknown write lets the certainty rule call a flow certain when it is not. **Defects 1 and 2 are ONE change and the order is forced** — `resolveWrite`'s `flatten` drops `Undefined` before a union can form, so the `Undefined`-arm case is currently *unreachable* and fixing (1) alone would create the exposure that measures zero today. Corpus payoff near zero (3 emissions); the case is correctness. Measure at the **inspection** level by dumping the residual — four graph-vs-inspection incomparabilities are on record in BUG-419/424 | ✓ |
