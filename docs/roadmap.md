...
| BUG-473 | `LuaTypesSnapshot.forFile` is superlinear in call-site count once a `---@class` is present | planned | M | — | profiled and planned: all of it is `VariableElement.resolveRead` re-walking a transitively-closed `downSet` per admitted pair (removing `resolveWrite`'s recursion entirely changes nothing). Two phases — a graph-revision-keyed root memo (semantics-preserving, measured 5x) then closure-flattening of the read walk (measured 39x, changes `read` on 1 of 817 nodes, gated on a differential harness) | ✓ |
