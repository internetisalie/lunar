...
| BUG-430 | `a.b.c = v` makes `c` a member of `a` and leaves `a.b` empty; the two enumeration doors disagree | todo | **M** | — | planned 2026-08-14 (plan-bug). **Re-grounded, and it shrank**: `Foo.bar` is empty because the hoist puts `baz` on `Foo`, so nothing ever constrains `bar` on *either* side — the "exact literal does not accrete" story describes a mechanism not yet reached. `isExact` has ONE semantic consumer (`LuaTypeGraph:735`, `Missing required field`) and that message is **absent from every corpus baseline**, so the exactness route costs far less than this row used to claim. Fix is sequenced: correct the suffix anchor at `visitIndexExpr:1134`, **re-probe**, then most likely give member enumeration the write+read merge `typeOf` already does (`LuaTypes.kt:170` reads `node.write`). Reproduction parked as `LuaNestedMemberAssignmentTest.kt.txt` | ✓ |
