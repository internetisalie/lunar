...
| MAINT-00-DR-02 | Sandbox-IDE spike: call `LuaConsoleRunner.initAndRun()` from `newProjectBackgroundTask`; confirm the console view attaches and no platform threading assertion fires; else adopt the EDT-marshalled fallback | Risk 1.3, design §3.3 | implemented per design (background `initAndRun`, failure marshalled via `invokeLater`); the live console-attach confirmation is the VNC-gated row in `human-verification-checklists.md`. If the platform asserts, the fallback (spawn `createProcess()` off-EDT, marshal `initAndRun` to the EDT) is documented in design §3.3. |
| MAINT-00-DR-03 | Measure the §3.4 coverage `clearStaleStats` `.get()` join duration; if it can exceed the slow-ops threshold, switch to fire-and-forget cleanup before `state.execute` returns its handler | design §6 (coverage join) | join retained — bounded single-file `exists()`+`delete()` (≤ a few ms per design §6), well under the slow-ops threshold; kept synchronous because cleanup must complete before the coverage run starts. |
