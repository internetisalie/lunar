...
| [`MAINT-33`](33-corpus-sweep/requirements.md) | **Corpus Sweep** | **S** | **Full** | Regression ratchet over three pinned real-world Lua projects (luacheck, luarocks, ZeroBrane; KOReader measured and parked on sweep time): parse errors, unresolved `require`s, inspection hits, per-symbol breakdown and non-Lua ballast measured headlessly and gated against committed baselines. Opt-in via `-PwithCorpus`, builder-only, never CI. All 6 phases shipped; found BUG-389, BUG-390, BUG-391, BUG-392 and BUG-393. One residual: the ballast `claimed` flag is all-or-nothing (verification Scenario 4.1), advisory and ungated. |
