...
| [`MAINT-03`](03-deprecation-cleanup/requirements.md) | **Deprecation Cleanup** | **L** | **Planned** | Remove all 33 `src/main` `@Deprecated`-API usages (DataManager, FileChooserDescriptorFactory, `runReadAction`→`runReadActionBlocking` ×14, misc singletons), retire Lunar's own deprecated `LuaProjectSettings.State.platform` prop, and bump the IntelliJ Platform Gradle plugin 2.5→2.17. Behavior-preserving; Phase 0 DR gate verifies unconfirmed replacements. |
