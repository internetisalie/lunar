...
| [`MAINT-20`](20-full-platform-syntax-migration/requirements.md) | **Headless Parser & Lexer Generation** | **C** | **Planned** | Follow-up to MAINT-19. **Scope pivoted 2026-07-02** away from platform.syntax/Kotlin emission (abandoned — JFlex/Grammar-Kit emit Java natively and that's the standard) to making lexer+parser generation **fully headless (no IDE handoff)**. Proven feasible: `org.intellij.grammar.Main` runs headless against the IDE-bundled grammar-kit jar; only a cosmetic `var`→`var_$` version diff. Wires jar resolution into `generate.sh` and removes the manual "human right-clicks the grammar" step from the agent guide. |
