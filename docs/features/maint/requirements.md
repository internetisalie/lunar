...
| [`MAINT-20`](20-full-platform-syntax-migration/requirements.md) | **Headless Parser & Lexer Generation** | **C** | **In Progress** | Follow-up to MAINT-19. **Scope pivoted 2026-07-02** away from platform.syntax/Kotlin emission (abandoned — JFlex/Grammar-Kit emit Java natively and that's the standard) to making lexer+parser generation **fully headless (no IDE handoff)**. Both generator jars relocated to gitignored `tooling/parser-gen/` + `generate.sh` resolver added (commit `4fe9792e`); remaining: grammar-kit version decision, clean-checkout verification, de-manualize the agent guide. |
