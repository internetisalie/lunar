...
| :-- | :-- | :--: | :--: | :-- |
| R1 | **The gate runs without a judge.** A sweep whose `luac` is missing could measure nothing while reporting success — the failure mode a ratchet can least afford. | Low *(was High)* | **High** | Reduced by **owning the dependency** (MAINT-35-00) rather than tolerating its absence: `lua5.1`–`lua5.4` are provisioned in all three paths, and a missing binary fails the sweep before it judges a file, naming the expected path and `fetch-luac.py`. **Strengthened again 2026-08-05**: existence is not liveness, so `ParseOracle.assertDiscriminates` now judges a known-good and a known-bad snippet before any sweep — a `luac` that answered Accept to everything would otherwise make `oracleDisagreements` structurally zero and leave the gate green while measuring nothing. An earlier design instead made the metric nullable and taught the ratchet to survive absence; that was more code, more cases, and strictly weaker — a gate that cannot run is louder than one that runs empty. |
...
| R7 | **Corpus runtime ceiling.** MAINT-33 parked KOReader for pushing the sweep past 10 minutes; this feature adds work to every file plus a new member. | ~~Medium~~ **Closed** | Medium | **Measured 2026-08-05, no action needed.** The torture member sweeps 1 696 inputs in **5.7 s** — cheaper than any project member by two orders of magnitude, because it does no indexing, no highlighting and no require resolution. The four project members total ~5.5 min, unchanged in kind by the oracle (one short-lived process per file). |
...
| R9 | **`ParseOracleTest` does not run in CI.** It needs the pinned binaries under `test/luac/`, which live in the out-of-repo `test/` tree a CI checkout does not have. The plan said "CI runs `fetch-luac.py` like the builder does" — but that builds five Lua releases on **every push**, ~2 min added to a job that already takes ~7. | Certain | Medium | Excluded via `-PexcludeExternalFixtureTests` (`build.gradle.kts`), alongside the two tests that already have this exact dependency. **Recorded rather than decided silently**: the oracle unit tests are covered on the builder, not in CI, until someone chooses to pay the fetch cost or cache `test/luac/` by manifest digest. The corpus sweep itself was never in CI either, so this does not narrow existing coverage. |
