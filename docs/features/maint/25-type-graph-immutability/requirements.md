...
| MAINT-25-04 | Error-reporting hygiene | S | Full | Cutoffs → `log.warn`; never log PCE (#14) |
| MAINT-25-05 | Snapshot-cost pass | C | Full | Fix the O(n²) cats-comment scan and per-access `nodes`-list copy (§2.5.5); `write`/`read` memo deferred |
| MAINT-25-06 | No-regression gate | M | Full | REDIS-04 §3.1c-style contract: full `lang/types/*` + consumers green on a cache-defeated full-suite run vs the 2123/0/1 baseline (measured 2128/0/1) |
