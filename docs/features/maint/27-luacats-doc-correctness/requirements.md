...
| MAINT-27-02 | Escaped, correct doc HTML | M | Full | `renderTypeText` HTML-escapes type text and links only simple identifiers `^[\p{L}_][\p{L}\p{N}_.]*$`; structured types render as escaped code (#35); `---@type T` local renders `local <name> : <type>`, not `class T` (#57) — design §3.2/§3.3 |
| MAINT-27-03 | Inheritance rendering | S | Full | `resolveClassComment` iterates individual `parentTypes.argTypeList` and falls back to `LuaCatsTypeNameIndex` for bare `@class` (#36); `collectInheritedFieldTags` BFS-walks the chain with a `visited` cycle guard + `<= 64` depth cap (#67) — design §3.4/§3.5 |
| MAINT-27-04 | Alias values | S | Full | Gate the alias `Values:` section on `comment.typeOptionList.isNotEmpty()` instead of `enumTagList` (#37) — design §5 Ex.3. NOTE: only the `---\|` continuation form populates `typeOptionList` (the inline `"r"\|"w"` shorthand parses as the aliasTag `ARG_TYPE` union) — TC-04 asserts the `---\|` form. |
| MAINT-27-05 | Direct-children getters | S | Full | All 26 `LuaCatsLazyCommentImpl` getters now delegate to the inner `LuaCatsComment` child (whose generated getters use `getChildrenOfTypeAsList` on direct children); no cache (#38, review §2.5.5) — design §3.6. DEVIATION: the lazy node's tags live one level below it (inner `COMMENT`), so a `this`-level swap failed; delegation preserves the #38 intent. See risks-and-gaps.md. |
| MAINT-27-06 | Annotator cleanup | C | Full | Fixture-verified current highlighting, then removed the dead `LuaCatsNamedType` classTag/aliasTag special case in `LuaCatsAnnotator` (#72) — design §3.7. DEVIATION: KEPT the `LuaCatsElementTypes.NAME` else-branch — the fixture proved it produces the class-name NAME highlight (raw NAME token), so removing it would break TC-06. See risks-and-gaps.md. |
