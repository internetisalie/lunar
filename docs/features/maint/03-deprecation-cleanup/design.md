...
|---|---|---|---|
| `lang/format/LuaCodeStyleSettings.kt:112,120` | `CodeStyleSettingsCustomizable.WRAP_OPTIONS`/`WRAP_VALUES` | `WRAP_OPTIONS` → `CodeStyleSettingsCustomizableOptions.getInstance().WRAP_OPTIONS` (verified present via `javap`). **`WRAP_VALUES` has NO equivalent on `CodeStyleSettingsCustomizableOptions`** (only `WRAP_OPTIONS`/`WRAP_OPTIONS_FOR_SINGLETON`/`BRACE_OPTIONS`/`WRAP_ON_TYPING_OPTIONS`); `WRAP_VALUES` (`int[]`) still lives only on the deprecated `CodeStyleSettingsCustomizable`. Per **R-3**, the `WRAP_VALUES` read is kept with a method-level `@Suppress("DEPRECATION")` + rationale. | **partially grounded — `@Suppress` for WRAP_VALUES (DR)** |
...
| `lang/completion/templates/LuaTemplateContextType.kt:7` | `TemplateContextType(String id, String name)` | **kept + `@Suppress("DEPRECATION")` (DR-04).** The single-arg ctor leaves `myContextId` null, and the `<liveTemplateContext>` EP (`plugin.xml:282`) carries no `contextId` attribute, so `getContextId()` would assert-fail at runtime. The clean migration needs a `plugin.xml` `contextId="LUA"` addition (out of scope, §7) → suppress instead (R-3). | **`@Suppress` (DR-04)** |
...
| `lang/doc/LuaDocSearchEverywhereContributor.kt:111,112` | `ProgressIndicatorUtils.yieldToPendingWriteActions()` / `runInReadActionWithWriteActionPriority(task, indicator)` (DR-03) | **`@Suppress("DEPRECATION")` + rationale.** All overloads are `@Deprecated`; the modern replacement is coroutine `readAction`/`smartReadAction` — a threading-semantics change (out of scope, R-2/R-3). | **`@Suppress` (DR-03)** |
| `coverage/report/LuaCovReportHighlight.kt:24,27` | `createTextAttributesKey(String, TextAttributes)` | **`@Suppress("DEPRECATION")` + rationale.** Confirmed `@Deprecated`; no non-deprecated overload takes raw `TextAttributes` (only `(String)` / `(String, TextAttributesKey)`). Sites deliberately pin colors for non-inheriting themes (existing in-code comment). | **`@Suppress` (DR-03)** |
