...
- **Goal**: Correct argument de-dup and the Lua 5.1 `arg` global — both pure-function, no
  annotator changes.
- **Tasks**:
  - [x] Edit `net.internetisalie.lunar.analysis.luacheck.LuaCheckCommandLine` — remove
  ...
  - [x] Edit `net.internetisalie.lunar.analysis.inspections.LuaStandardGlobals` — add `"arg"`
...
- **Tasks**:
  - [x] Edit `net.internetisalie.lunar.analysis.inspections.LuaInspectionSuppression` —
  ...
  - [x] Same file — `parseLuacheck` (`:135`) range end changes from `minOf(commentLine + 1, lineCount)`
        to `commentLine` (§3.6).
...
- **Tasks**:
  - [x] Create `net.internetisalie.lunar.analysis.luacheck.LuaCheckOutcome` (§2.1) —
        `sealed interface` + `FailureKind` enum.
  - [x] Edit `net.internetisalie.lunar.analysis.luacheck.LuaCheckInvoker` — `invoke` returns
...
- **Tasks**:
  - [x] Edit `LuaCheckAnnotator.Info` (§2.4) — carry `fileName`, `workDir`, `documentText`,
  ...
  - [x] Edit `newLuaCheckCommandLine` (§2.3) — add `useStdin: Boolean = true`; when set, emit
        `-` positional + `--filename <targetFileName>`.
  - [x] Edit `LuaCheckInvoker.invoke(info)` — call `capture(cmd, FORMAT, stdin = info.documentText)`.
  - [x] Edit `LuaCheckAnnotator.doAnnotate`/`apply` — consume `LuaCheckOutcome`; `applyProblem`
...
## Verification Tasks
- [x] `LuaCheckCommandLineTest` — extend with `dedupePairs` cases — covers TC1, TC2.
- [x] New `LuaCheckInvokerClassifyTest` — stub `LuaExecResult` outcomes — covers TC10–TC12.
- [x] New/extended `LuaInspectionSuppressionTest` (`BasePlatformTestCase`, `configureByText`) —
      covers TC6, TC7.
- [x] New `LuaStandardGlobalsTest` — covers TC8, TC9.
- [x] Extended `LuaCheckAnnotatorTest` — covers TC3, TC4, TC5 (annotator drives a fake exec via
      the DR-02 seam).
...
- [x] Full-suite gate via `gce-builder`; regression-relative to baseline 2123/0/1 (main 590acc29).
