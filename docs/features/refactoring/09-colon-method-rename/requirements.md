...
| `REFACT-09-05` | **A declaration outside the project is refused** | M | Full | A colon call that resolves outside the project is refused instead of rewritten. **Which layer refuses depends on the input, and only one of them names the file** ([[BUG-480]]): a jar-backed stdlib stub, and a protected non-project file, are refused by the platform before Lunar is asked; a non-project file the user has unlocked for editing reaches design §3.6's guard, which names the declaring file. Outcome is a refusal in all three. |
...
| 14a | `REFACT-09-05` | `local f = io.open("x")` ; `f:<caret>write("y")` | rename through `PsiElementRenameHandler.invoke` (the rename ACTION) | refused, file byte-identical, message is the PLATFORM's `error.cannot.be.renamed` and does NOT name `io.lua` | **(executed)** drive it with `myFixture.renameElementAtCaret` instead → the refusal becomes Lunar's and both assertions redden. That mutation is [[BUG-480]]: `canRename` runs at `PsiElementRenameHandler.java:114`, the processor at `:132`, and a jar-backed element never reaches the second — measured, the probe inside `substituteElementToRename` printed nothing |
| 14b | `REFACT-09-05` | `function t:m() end` in a `.lua` file under no content root, non-project write protection lifted | rename through `PsiElementRenameHandler.invoke` | the platform ALLOWS the element (`canRename` returns true) and design §3.6's guard refuses, naming the declaring file | **(executed)** delete the `outOfProjectRefusal` call from `colonMethodSubstitution` → nothing declines, `invoke` opens the rename, this reddens while 14a stays green. This is the input at which the guard is the deciding layer; at a jar stub it is shadowed |
