...
| Caret on the receiver `M` of `function M.run()` when `M = {}` also exists | **Not** refused, and never reaches step 4a: `TargetElementUtilBase` tries `REFERENCED_ELEMENT_ACCEPTED` first, `resolve()` lands on the `M` of `M = {}`, and rename proceeds against that leaf (§3.5 row 14) with the funcName's `M` collected as an ordinary usage. |
...
| Two locals of the same name in nested blocks | Handled by resolution, not by rename: `LuaNameReference.isReferenceTo` re-resolves each candidate instead of matching text (`LuaNameReference.kt:233-244`), which is why REFACT-01-03 costs nothing extra. |
