...
| `LuaCatsTypeReference` + one `psi.referenceContributor language="Lua"` **and an override on `LuaCatsBaseElement`** | cats PSI reports `LuaLanguage` (`LuaCatsElementType.kt:23`) and resolves via `LuaCatsTypeNameIndex` (P2) — but the contributor **alone is inert**, and that half of this row was read from the constructor rather than run. `LuaCatsBaseElement` does not override `getReferences()`, so it inherits `PsiElementBase.getReferences()` → `SharedPsiElementImplUtil.getReferences`, which never consults `ReferenceProvidersRegistry`. Measured in [[REFACT-08]] `REFACT-08-00-DR-02`: contributor registered and pattern matching, `references=0`, and a rename that moved the `@class` and left all eleven uses stale. `REFACT-08-14` is the missing override |
...
| a `LuaFindUsagesProvider.canFindUsagesFor` branch **and a `referencesSearch` executor** | **a searcher IS needed**, and this row's original "no searcher needed — the default `ReferencesSearch` word scan already reaches every cats use site (P7)" is measured false. `CachesBasedRefSearcher` derives a search text only for a `PsiFileSystemItem`, a `PsiNamedElement` or a `PsiMetaOwner` (`CachesBasedRefSearcher.java:26-56`); a bare cats `NAME` leaf is none of the three, so its text is null and nothing is scanned. Measured in [[REFACT-08]] `REFACT-08-00-DR-03` mutation O — with everything else in place and only the `referencesSearch` registration removed, every fixture reports `references=0` and a rename moves the declaration while leaving both uses stale (`staleWidget=2 newGadget=0`). `LuaCatsTypeReferenceSearcher` is that executor |
