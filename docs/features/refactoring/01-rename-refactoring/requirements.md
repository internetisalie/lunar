...
- **`REFACT-01-08`** — the dotted form ships. `REFACT-01-00-DR-03` measured the colon form and found
  ...
  shapes. Advancing it needs class inference that does not exist, and is gated on [[BUG-473]].
...
- **`REFACT-01-09`** and **`REFACT-01-11`** — `C`, `Future Work`, each with a structural blocker
  named in its row (a grammar change; a level-aware lexer). Neither is delegated, because their
  natural owners are a closed feature and an epic with no open member — delegating to either would
  orphan the work rather than route it.

**What is deliberately not claimed.** `-19` and `-20` are `Won't` by analysis, not by omission:
`self` / `...` and dynamic `_G["x"]` access are out of reach for a static rename.
