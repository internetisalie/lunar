...
| `REFACT-01-08` | **Rename a method or dotted member declaration** | **S** | **Partial** | **Audited before Phase 2:** `function Obj:m()` / `function Obj.m()` — `canFindUsagesFor` accepts `LuaFuncName` and `LuaFuncNameMethod` grandparents, so declarations are findable. Note the known resolution caveat that constrains any implementation: references key on receiver *text* (`b.setName`), so `local b = Builder; b:setName()` does not resolve to `function Builder:setName` and would be missed. **Phase 2 (2026-08-23):** Deliberately **refused**, not attempted: `function Obj:m()` with `refactoring.rename.colonMethod` and a function-name receiver segment with `refactoring.rename.functionNameSegment` (TC-19a, TC-34a, TC-34b). Phase 4 owns the dotted form. **Phase 4 (2026-08-23):** The dotted form renames with its call sites — `function M.<caret>run()` → `start` rewrites the declaration *and* every `M.run()` (TC-09) — and the colon form is refused by name (TC-10, asserting the `refactoring.rename.colonMethod` message and a byte-for-byte unchanged file, not merely that something declined). **Partial, not Full, and the missing half is deliberate:** `function Obj:m()` stays refused because `findReferences` on it returns **zero** references — measured — so renaming it would half-apply. Receiver-type-based method resolution is DR-03 in `risks-and-gaps.md`; until it exists the refusal is the correct behaviour, and the row cannot claim the requirement's first noun. Phase 4 added no production code for this row: §3.1 step 4b shipped with Phase 2 and the `DOTTED_FUNCTION` path needed nothing, so the phase's work was executing the claim and pinning it. **DR-05 (2026-08-23) found a second, independent reason this row is not `Full`:** the dotted form renames from its DECLARATION caret, but a caret on an `M.run()` **call site** is refused by the platform (`error.cannot.be.renamed`) before this processor is consulted — `TargetElementUtil` hands back the whole `LuaFuncDecl`, which `canProcessElement` deliberately does not claim (Gap 2.10). Measured, contained and safe — never a half-rename — and recorded as Gap 2.14, filed 2026-08-23 as [[BUG-465]] because it outlives this feature and needs the same `TargetElementEvaluatorEx2` Gap 2.9 needs. **Phase-4 remediation (2026-08-23):** the dotted rename this row does claim was also passing conflict detection blind — see `REFACT-01-14` — which is now fixed and pinned; it changes no claim here, but it is what makes the claim safe. **BUG-465 closed (2026-08-29):** the call-site half of that is fixed — `M.ru<caret>n()` now renames the declaration and every call site, through `LuaTargetElementEvaluator.adjustTargetElement`, which maps the resolved `LuaFuncDecl` down to the leaf it declares only when the reference at the caret names that same segment (Gap 2.10 stays shut; the check is mutation-proven against `testCaretOnAGlobalShadowedByADottedDeclarationIsRefusedNotMisdirected`). Pinned by `testDottedFunctionCallSiteCaretTargetsTheDeclarationsNameLeaf`, `testRenamingADottedFunctionFromACallSiteRewritesDeclarationAndUsages` and the receiver control `testDottedCallSiteReceiverCaretIsNotRedirectedToTheNameSegment`. **The row stays `Partial`, now for exactly one reason:** the colon form `function Obj:m()` is still refused because `findReferences` on it returns zero references, pending DR-03's receiver-type-based method resolution. It cannot claim the requirement's first noun until that exists. **DR-03 executed (2026-08-29) and its outcome is that the row stays `Partial` for longer than the sentence above implies.** Receiver-type method resolution *exists*: `LuaTypeManager.resolveType(className, file)` returns a `LuaClassType` whose `resolveMember(name).sourceElement` is the `function Class:method` `LuaFuncDecl`, cross-file included, and a throwaway prototype renamed `local b = Builder; b:setName()` end to end — the exact case this row's caveat clause calls unreachable. What is missing is a `className` for un-annotated Lua. It is minted only from a cats tag, so a plain table, a global table, `setmetatable` OO and an un-annotated module return all infer null, and with the `METHOD_FUNCTION` refusal lifted each of those SILENTLY HALF-RENAMED (measured: `function Obj:renamed() end` beside an untouched `o:m()`). Corpus density says which case dominates: **0 of 734 files across the four pinned corpora carry a `---@class`, against 809 colon-method declarations and 16,336 colon-call sites.** The verdict is a **TYPE-epic dependency, not a REFACT-01 phase** — the missing component is class inference from metatable/module-return OO — and the cats-only follow-up feature is not worth cutting alone because it would refuse on every real file, i.e. change nothing a user sees, while putting `LuaTypesSnapshot.forFile` on the resolve hot path. The refusal shipped in Phase 4 remains correct. Full evidence, the seven-shape table, the corpus densities, the cost measurements and the eight-line `self` guard are in `risks-and-gaps.md` under "DR-03 result". |
...
| `REFACT-01-16` | **Rename propagates into LuaCATS annotations** | **S** | **Partial** | The `@param` half ships (Phase 6): renaming a parameter moves its `---@param` tag. **The type-name half is delegated to [[REFACT-08]]**, not duplicated. `REFACT-01-00-DR-04` sized it and the reason it is not a phase is structural: of 70 elements spelling a type name, **zero** answer `getReference()` or `getReferences()` and none is a `PsiNamedElement`, so there is no symbol to rename — the work is to create one, upstream of everything this feature built. The index route was prototyped and is a guaranteed **silent** half-apply: it rewrites the declaration, reaches no use, and demotes a documented type to prose (`LuaCatsDeclaredType.isType` true to false) with nothing reported. This row becomes `Full` when REFACT-08 ships. |
