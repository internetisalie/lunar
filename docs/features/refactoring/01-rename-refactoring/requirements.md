...
| `REFACT-01-16` | **Rename propagates into LuaCATS annotations** | **S** | **Full** | The `@param` half ships (Phase 6): renaming a parameter moves its `---@param` tag. **The type-name half shipped as [[REFACT-08]]**, not duplicated here — see that feature's `requirements.md` for the design, the test cases and the refusal set (a builtin-keyword name, a parameterized class head, a name any attached library also declares). |
