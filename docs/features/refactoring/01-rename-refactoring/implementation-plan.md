...
| TC-10 | -08 | `LuaRenameTest.testColonMethodDeclarationWithAnAliasedCallSiteReportsAConflict` | `Obj = {}`<br>`function Obj:<caret>m() end`<br>`local o = Obj`<br>`o:m()` | `myFixture.renameElementAtCaret("renamed")` | throws `BaseRefactoringProcessor.ConflictsInTestsException` whose message contains `"cannot be bound to a declaration"`; file text unchanged. **Rewritten by [[REFACT-09]] Phase 4**: the blanket refusal this row asserted (`refactoring.rename.colonMethod`, thrown from `substituteElementToRename`) is deleted. A colon method is renamable now, and the aliased `o:m()` is reported as a conflict rather than the whole rename being declined. |
