...
| `REFACT-07-14` | **The numeric-`for` variable is not offered the template** | **C** | **Full** | With the caret on `i` in `for i = 1, 10 do`, no inline template starts; the user gets whatever the dialog path gives them. There is no `nameRef` to anchor a template on. **Unchanged by [[BUG-469]] (2026-08-29)** — that fix gave this caret a rename *target*, so the dialog path now works where it previously reached no handler at all, but no in-place handler claims it and none may. |
...
| TC-12 | `REFACT-07-14` | `for i<caret> = 1, 10 do`⏎`  print(i)`⏎`end` | ask the registry for available handlers | exactly one handler, and it is `PsiElementRenameHandler` — no in-place handler is available | **Rewritten for [[BUG-469]] (2026-08-29); half guard, half proven.** It previously asserted an **empty** handler list, on the measured ground that the data context supplied **null** here (DR-05 probes `f`/`f2`/`f3`). That ground was BUG-469 — a defect, not a property — and `LuaTargetElementEvaluator.getNamedElement` closed it, so the context now supplies the control variable's IDENTIFIER leaf and the platform's dialog handler claims it. The **count** assertion is mutation-proven: dropping that override reddens it (`expected:<1> but was:<0>`, executed). The **claimant-identity** assertion remains a **guard** — `LuaInplaceRenameHandler.declaringNameRefOf` ends at `leaf.parent as? LuaNameRef` and `numericForStatement ::= FOR IDENTIFIER '='` (`lua.bnf:152`) gives a numeric-`for` leaf no `LuaNameRef` parent, so no edit short of a grammar change lets an in-place handler claim this caret. `risks-and-gaps.md` Risk 1.7 carries the argument. |
