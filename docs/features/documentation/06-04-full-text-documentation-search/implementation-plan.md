...
- **Tasks**:
  - [ ] **Replace `LuaDescriptionIndex.Indexer.map()`** (`lang/indexing/LuaDescriptionIndex.kt:50–62`) — implements design §2.1. The new indexer uses `PsiTreeUtil.findChildrenOfType(psiFile, LuaCommentOwner::class.java)` to visit every documentation-attached element, extracts description text via `collectDescriptionText(comment)`, tokenizes, and emits `word → "qualifiedName\tfileUrl\ttextOffset"` entries.
  - [ ] **Add `collectDescriptionText` helper** (`internal` top-level function in `lang/syntax/LuaComment.kt`) — realizes design §3.1. Iterates `comment.descriptionList` and every tag list with `getDescription()`, concatenating all text. Shared by both the indexer and the contributor's re-check.
  - [ ] **Bump `getVersion()` from 1 to 2** — triggers re-indexing on next IDE start.
- **Exit criteria**:
...
- **Tasks**:
  - [ ] **Create `LuaDocSearchItem`** (`lang/doc/LuaDocSearchItem.kt`) — realizes design §2.2. Implements `NavigationItem`, stores `project`, `symbolName`, `fileUrl`, `declarationOffset`. `getPresentation()` returns name + relative path + icon. `navigate()` opens file at offset.
  - [ ] **Create `LuaDocSearchEverywhereContributor`** (`lang/doc/LuaDocSearchEverywhereContributor.kt`) — realizes design §2.3. Implements `SearchEverywhereContributor<LuaDocSearchItem>` + inner `Factory : SearchEverywhereContributorFactory`. `fetchElements()` tokenizes pattern, looks up first token in index, deduplicates, re-checks multi-word patterns, processes matching items.
  - [ ] **Add `descriptionContainsAllTokens` helper** (private to contributor) — realizes design §3.2. Loads PSI, extracts full description text using the shared `collectDescriptionText()`, confirms all tokens are present.
  - [ ] **Register in `plugin.xml`**: `<searchEverywhereContributor implementation="net.internetisalie.lunar.lang.doc.LuaDocSearchEverywhereContributor$Factory"/>`.
- **Exit criteria**:
...
- **Tasks**:
  - [ ] Verified in Phase 2's `LuaDocSearchItem.navigate()` implementation.
- **Exit criteria**:
...
- **Tasks**:
  - [ ] Verify `LuaDocSearchItem.getPresentation()` returns correct `ItemPresentation` with `presentableText` (symbol name), `locationString` (relative path), and non-null icon.
- **Exit criteria**:
...
## Verification Tasks
- [ ] **Unit: `LuaDescriptionIndexTest`** — configures Lua files with `@class`, `@param`, `@return`, `@field` descriptions, asserts exact index keys and values. Covers TC 1, 8.
- [ ] **Unit: `LuaDocSearchEverywhereContributorTest`** — configures Lua files with doc comments, calls `fetchElements` with single and multi-word patterns, asserts returned item count and names. Covers TC 2, 3, 4, 5.
- [ ] **Integration: dumb-mode safety** — verifies `fetchElements` returns immediately when project is dumb. Covers TC 9.
- [ ] **Integration: navigation** — selects a result, asserts editor opens at correct offset. Covers TC 6.
- [ ] **IDE verification** (manual, via `verify-in-ide` skill): open Search Everywhere in sandbox GoLand, type a documentation keyword from test project, confirm results appear and navigate correctly.

## Task Summary

| Phase | Status | Priority |
|-------|--------|----------|
| Phase 1: Complete LuaDescriptionIndex indexer | todo | Must |
