...
| 14 | `NAV-13-05` | `local t = {}` ; `function t:m() end` ; `local m = 1` ; `t:m()` ; `print(m)` | as #1 | the leaf `m@24`, **not** the local `m@38` | **not** as #13 — corrected in Phase 2 by execution. The fall-through mutation is **structurally unreachable from this fixture**: it only changes behaviour where `declarationLeafOf` returns null, and here the branch *succeeds* at `m@24`, so the mutated line never runs (executed, the row stayed green). The reachable falsifier is #1's — delete the colon branch — **executed on this row's own test**, which reddened at `expected:<24> but was:<38>`, exactly the `LeafPsiElement@38` the pre-change measurement transcribes. No clause of `declarationLeafOf` separates this row from #1: the branch is exclusive by construction, so there is no choice point at which it could prefer the local |
...
| 16 | `NAV-13-06` | `local t = {}` ; `function t:m(n) end` and 80 × `t:m("aN")` | build the snapshot, then resolve every call site twice | all 80 resolve on both passes; `RootAccessor.WRITE` / `READ` stay within the budgets `implementation-plan.md` Phase 3 commits — **executed**, `WRITE` = 165 and `READ` = 83, committed as 180 / 92 | remove the `isSnapshotUnderConstruction` guard from `declarationLeafOf` (design §3.6) — measured in DR-02 to take the *annotated* 80-site fixture's `WRITE` from 572 to 812 and redden the budget methods that predate this feature |
...
| 20 | `NAV-13-06` | the DR-04 ring fixture — `K` files, file *i* declaring `---@class Ci` with method `mi` and carrying 10 call sites on a `---@type C(i+1 mod K)` receiver — at `K` = 2, 4, 8, 16 | resolve every site, then sum `rootResolutionCount(WRITE)` and `(READ)` over **every** file's `LuaTypesSnapshot` | every site resolves, and the summed `WRITE` and `READ` each exceed the K-proportional baseline by **exactly one per resolved site**, at every K — so the per-site delta does not grow with `K` | any change that makes the per-site delta a function of `K`. **The guard removal (as #16) was executed against the shipped test and does NOT redden it** — the un-guarded branch inflates the *baseline* (snapshot construction), and the shipped form asserts a before/after **delta**, which cannot see that; cases 16 and 17 carry that falsifier. The reachable falsifier here is case 1's — delete the colon branch from `LuaNameReference.multiResolve` — **executed**, red at `expected:<20> but was:<0>` on the anti-vacuity assertion. The absolute totals were **executed** pre-feature and under the prototype at every `K` listed (`risks-and-gaps.md` DR-04), and the shipped code reproduced them value for value |
...
- [x] Every de-risking action listed under "De-risking" below has run and its result is recorded in
      `risks-and-gaps.md`.
- [x] Every `M` requirement has an executed test case with a named, reachable mutation — except the
...
- [x] `LuaTypeGraphRootResolutionBudgetTest`'s methods that predate this feature pass **at their
      committed budgets, unchanged** (`NAV-13-06`).
- [x] The new un-annotated budget method passes at the value `implementation-plan.md` Phase 3
      commits (`NAV-13-06`).
- [x] The full unit suite is green (`NAV-13-05`).
...
- [x] Case 19 passes and its mutation was observed reddening it (`NAV-13-07`'s exception).
- [x] Case 20 passes at every `K` it covers, so the cross-file fan-out is bounded rather than merely
      unobserved (`NAV-13-06`, `risks-and-gaps.md` Gap 2.3).
- [x] The downstream consumer set was re-enumerated **by execution** against the shipped code, by the
...
- [x] The `plugin.xml` surface set was re-derived by **both** halves of `design.md` §7's rule — the
...
- [x] The mirror sweep `risks-and-gaps.md` DR-06 specifies was re-run against the shipped code — every
...
- [x] Every `NAV-13-08` case passes — cases 21-24 and 31-32 on the call site's side, cases 25-30 and
