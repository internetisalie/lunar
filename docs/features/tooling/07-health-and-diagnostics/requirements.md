...
| TOOLING-07-02 | **Registry-mediated health writes** | M | Full | Health results are written only through `LuaToolchainRegistry` mutation methods, which fire `LuaToolchainListener.TOPIC`. No read path mutates health (removes the `LuaToolManager.getTools()` anti-pattern). |
