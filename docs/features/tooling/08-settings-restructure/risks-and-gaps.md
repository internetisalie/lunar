...
| #44 | `LuaApplicationSettingsConfigurable` has no `reset()`/`disposeUIResources()`; panel mutates live persisted `LuaInterpreter` objects pre-Apply — Cancel can't undo | DSL migration phase: clone-edit-commit pattern + full Configurable lifecycle | **CLOSED (Phase 5).** The `LuaInterpreter` table premise was stale (removed in TOOLING-05); the page now edits two `LuaApplicationSettings.State` booleans. `LuaApplicationSettingsConfigurable` gained `reset()` + `disposeUIResources()` (drops the panel ref); the DSL-rebuilt `LuaApplicationSettingsPanel` is the buffer — its checkboxes only commit to the live state in `getData` (called solely from `apply()`), so *Cancel* never touches persisted state and `isModified` compares UI vs live honestly. Evidence: `LuaApplicationSettingsPanelTest.testIsModifiedAfterTogglingTypeInference_TC11`. |
