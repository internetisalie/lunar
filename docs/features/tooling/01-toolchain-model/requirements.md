...
| TOOLING-01-02 | **Built-in kind registry** | M | Full | `LuaToolKindRegistry` ships the built-in kinds enumerated by `LuaToolKindRegistry.BUILT_IN` with complete probe specs; lookup by id is O(1); the set is a data list. (As of 2026-07-17 that is 11 kinds: `lua`, `luajit`, `tarantool`, `luarocks`, `luacheck`, `stylua`, `luacov`, `busted`, `redis-server`, `valkey-server`, `lua-language-server` — the last two added by REDIS, the last by BUG-373. `BUILT_IN` is authoritative; don't hardcode the count.) |
