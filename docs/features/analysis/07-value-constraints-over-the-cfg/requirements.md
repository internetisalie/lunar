...
| ANALYSIS-07-03 | Constraints scoped to a subgraph | Should | Not Implemented | A value constraint that holds on one branch and not its sibling, reaching **members** and not only the bare type — the [[BUG-435]] shape. **Gated on `ANALYSIS-07-00-DR-10`**, which may descope it entirely. |
