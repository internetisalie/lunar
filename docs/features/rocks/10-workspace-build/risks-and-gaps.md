...
|----|--------|----------|--------|
| ROCKS-10-DR-01 | Confirm ROCKS-09's `discoverRockspecPaths()`/`DiscoveredRockspec` shape against the merged ROCKS-09 implementation before starting Phase 2 (the contract is LOCKED in ROCKS-09 design, but verify the symbol exists post-merge). | Risk 1.1 | todo |
| ROCKS-10-DR-02 | Inspect the `Kernel/v0` `rocks/*/` set for duplicate `package` names and confirm the inter-rock dependency edges (which rock depends on which) match the expected A→B→C-style chain used in tests. | Gap 2.1, ROCKS-10-02 | todo |
