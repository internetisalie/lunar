...
| TYPE-11-06 | **An incomplete recording is never pinned** | M | The generalisation of TYPE-11-05, and the reason it is a requirement of its own: the recorder must distinguish **"no sources"** from **"sources unknown"**, and only the first may be pinned. A build during which a global resolution answered nothing, or during which a nested `forFile` was served from cache without contributing its own recorded sources, has not established that it is a pure function of provisioned content. Measured (design §1.8): both under-recordings ship a stale type under the rule as originally written, and closing them costs **zero** of the 11 files this feature pins. **Four channels, all measured**: an absence (B1), a warm inner snapshot (B4), an **in-progress** inner snapshot (V1) and a **rescued** global — one the project scope did not answer but the all-scope fallback did, which a later project declaration out-ranks (V2). Each was reproduced red and each fix costs **zero** of the 11 pinned files (§1.8, §1.10). The **fifth** — `resolveModule`'s `ANY` fall-through, which records nothing when a `require` resolves to nothing (design §1.12) — is closed on correctness, and **its cost is measured too** (DR-18, Phase 2): `REVIEW-COST TOTALS provisioned=11 withModuleRule=11 withoutModuleRule=11`, zero lost pins, so it is no longer the one rule adopted without a price. The **sixth** — a `LuaTypeReference` already forced at `depth() == 0`, consumed inside a build frame by `fromLuaType` without reaching `resolveType`, so the file it resolved into never entered `urls` and the library file was pinned while depending on it — was found in Phase 4 (DR-07), filed as **BUG-434**, and **closed 2026-08-12** by giving the memoized answer its frame and replaying it on every read (design §3.1 step 7). It is the only one of the six not closed by a clause of §3.3, because it is a *missing report* rather than a frame state: a clean frame that is wrongly clean cannot be told from a correct one by anything reading the frame. Cost, re-measured because this one is on the hot path: **zero** pins (`TYPE11-COST provisioned=11 pinnable=11`), routine suite 2630/0/1, corpus suite 2638/0/1, DR-04 arm B within noise. Gated by `TypeElevenDr11LateDeclarationTest`, `TypeElevenDr12WarmInnerSnapshotTest`, `TypeElevenDr14InProgressTest`, `TypeElevenDr15LateLibraryAnswerTest`, `TypeElevenDr18ModuleAbsenceTest` and `TypeElevenDr07LazyReferenceProbeTest` (arms 2 and 3 red under the named mutation, arms 1 and 4 the cold controls that stay green). **Satisfied.** |
...
### Phase 4b (2026-08-12) — BUG-434, and TYPE-11-06 is satisfied

Phase 4 left one `Must` unsatisfied: it *found* the sixth under-recording channel and deliberately
did not fix it, committing the reproduction as prints so that recording the defect would not cement
it. Phase 5 closes it.

- **The fix is Phase 2's idiom applied one layer down.** `LuaTypeReference` memoizes
  `Pair<LuaType, SourceFrame>` through `recording` and replays the frame on every read of `resolved`
  — so a reference forced at `depth() == 0` contributes to a later frame exactly what a cold read
  would. `isPinnable` is untouched; there is no step 8, because a missing report leaves a frame that
  is genuinely clean and no clause reading a clean frame can distinguish it (design §3.1 step 7,
  §3.3).
- **The reproduction is now a gate.** `TypeElevenDr07LazyReferenceProbeTest`'s four arms assert.
  Under the mutation "drop the replay from `resolved`, change nothing else", arms 2 and 3 go red with
  the stated messages and arms 1 and 4 — the cold controls — stay green, which is the split that
  makes them evidence rather than decoration.
- **Priced, because it is a hot-path change in a performance feature.** `TYPE11-COST provisioned=11
  pinnable=11` (zero pins lost); routine `ktlintCheck test` **2630 / 0 / 1**; corpus
  `test -PwithCorpus` **2638 / 0 / 1** (the `bf715eb2` baseline of 2631 plus Phase 4's 7 new
  tests; Phase 4b adds none); DR-04 arm B **+5.7 %** on paired isolated runs
