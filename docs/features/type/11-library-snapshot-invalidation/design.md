...
⚠⚠ **MEASURED FALSE IN PHASE 3, and the paragraph above is kept as the record of why.** Run against a
build whose pinnable branch emits exactly `Result.create(snapshot, psiFile, targetTracker)`, **TC-2c
passed** — with TC-1, TC-2a, TC-3, TC-15 and every residual fixture. The confound this section
removed was `MODIFICATION_COUNT`; the one it left is **`psiFile`**, which is a dependency in *both*
branches. Probed on the same `PsiFile` instance across `makeRootsChange`:
...
§1.6 had already recorded this exact mechanism for the dumb-mode exit and this section did not apply
it. No behavioural fixture can separate the two, because every route to a
`ProjectRootModificationTracker` tick runs through `makeRootsChange`, which is what moves the stamp.
The wiring is therefore made **directly** assertable, in §1.9 B5's idiom: §2.3 gains
`internal fun dependenciesFor(psiFile, frame): Array<Any>`, `forFile` spreads it, and **TC-2d**
asserts all three members by identity. TC-2c is kept — it is the behavioural statement of the
requirement — but it is not the gate this section says it is.
...
4. **TC-2d, the wiring — the one that actually fires.** `dependenciesFor(libraryFile, itsFrame)`
   contains the generation tracker, `targetModificationTracker` and the file, and nothing else. Red
   under both "churn object dropped from the pinnable branch" and "`targetTracker` dropped from the
   pinnable branch"; TC-2c is green under the first of those.
...
- **Key API**: `forFile`'s signature is unchanged — `fun forFile(file: PsiFile): LuaTypes` — and one
  ...
  internal fun dependenciesFor(psiFile: PsiFile, frame: LuaTypeSourceRecorder.SourceFrame): Array<Any>
  ```
  ⚠ **`dependenciesFor` was added in Phase 3 and is the outermost of the three.** `forFile` calls
  only it — `Result.create(builtTypes, *dependenciesFor(psiFile, sourceFrame))` — so the complete
  dependency set is one value a test can evaluate. It exists because §1.11's TC-2c was measured
...
- **Rules / edge handling**:
  - ⚠ **Steps 5, 6 and §3.7 have no *outcome* test that can go red, and Phase 3 measured it.**
    `TypeElevenDr12WarmInnerSnapshotTest` and `TypeElevenDr14InProgressTest` both stay green with
    their own guard deleted, because **step 7 subsumes them**: a library global that only another
...
| TC-18 | TYPE-11-06 (in-progress inner) | Library `outer.lua` = `OuterSeed = projectSeed` + `OuterGlobal = InnerSeed`, library `inner.lua` = `InnerSeed = OuterSeed`, project `p.lua` = `projectSeed = { beforeEdit = 1 }`. Resolve `OuterGlobal` (drives the cycle), then rewrite `p.lua` to `{ afterEdit = 1 }` with the roots tracker asserted still. | `resolveGlobal("InnerSeed").getMembers().keys` = `[afterEdit]`. Under §3 without step 6 it is `[beforeEdit]` — §1.10 V1. `TypeElevenDr14InProgressTest`. ⚠ **Green with step 6 deleted on the shipped build** — step 7 rejects this fixture first (§3.3 ⚠). The gates are `TypeElevenIncompleteFrameDecisionTest.testAnInProgressHitDeniesThePin` and `…testAMutualLibraryCycleRecordsAnInProgressHit`. |
...
| TC-20 | TYPE-11-06 (module absence) | Library `mu.lua` = `muAlias = require("mymod")` installed while **no** file provides `mymod` (asserted: `resolveModuleCandidates(project, "mymod")` is empty); read `muAlias`; then create project `mymod.lua` returning a table, with the roots tracker asserted still. | `resolveGlobal("muAlias")` reflects the new module. Under §3 without step 5d the frame is empty, the file is pinned, and it keeps `ANY` — §1.12. `TypeElevenDr18ModuleAbsenceTest`, plan Phase 3. ⚠ **Green with step 5d deleted** — `require` is itself a rescued global, so any file calling it is unpinnable before this rule is read (probed: `rescued=[global:require]`). The gate is `LuaTypeManagerRecordingTest.testAModuleThatResolvesToNothingIsRecordedAsAnAbsence`. |
