...
|----|-------------|----------|--------|-------------|
| `TYPE-13-01` | **A structural member carries its declaration** | M | Full | Landed Phase 3 (`implementation-plan.md`): `LuaTypesSnapshot.putGraphMember` wires `LuaMemberDeclarations.declaringNodeOf` into both `tableToLuaType` branches. `Type13MemberSourceElementTest` pins case 1 (`sourceElement = LuaFuncNameMethod@23`); the restore-two-argument mutation was executed and observed red. |
| `TYPE-13-02` | **The declaration mark is established at the mint site** | M | Full | `VariableNode.declaresMember` is set where the member node is created and is never inferred from `TypeNode.element`'s PSI class. A member's answer to *do you have a declaration?* is `sourceElement != null`; there is no separate discriminator field. Landed Phase 1 (`implementation-plan.md`); `Type13ProvenanceTest` pins all eight mint-site rows, each mutation executed and observed red. |
| `TYPE-13-03` | **A declaration-bearing member resolves to the declaration** | M | Full | `LuaMemberDeclarations.declarationOf` implemented and pinned by `Type13DeclarationLookupTest` (Phase 2); wired into production by Phase 3's `putGraphMember`, so `LuaTypeMember.sourceElement` now carries the declaration end to end. |
| `TYPE-13-04` | **A member with no declaring node is never offered as a declaration** | M | Full | `declaringNodeOf`/`declarationOf` refuse correctly, pinned by `Type13DeclarationLookupTest` (Phase 2); Phase 3 wires the same refusal into production `LuaTypeMember.sourceElement` via `putGraphMember`, so a member with no declaring node resolves with `sourceElement == null` in the ordinary `LuaType` API, not just in the standalone lookup. |
| `TYPE-13-05` | **`setmetatable` OO resolves to its declaration** | M | Full | `declaringNodeOf` recovers the declaration through `upSet`, pinned by `Type13DeclarationLookupTest` (Phase 2); wired into production by Phase 3, so `resolveMember("m")` on a `setmetatable` receiver now carries `sourceElement` pointing at `function Class:m()`, never the call-site `LuaMethodExpr`. |
| `TYPE-13-06` | **The nominal route is not regressed** | M | Full | Landed Phase 3: `putGraphMember`'s nominal-preservation guard (design §4.3) is unconditional, and `LuaUnionType.resolveMember`/`getMembers` (design §4.4) carry `sourceElement` through both entry points. `Type13MemberSourceElementTest` pins cases 8, 9, 10 and 17, each alone in its own fixture project; the guard-deletion and two-argument-`LuaTypeMember` mutations were executed and observed red for the exact values design §4.3 measured. |
| `TYPE-13-07` | **Cycle and depth safety** | M | Full | `declaringNodeOf` visits each node at most once (identity `visited` set) and stops at `MAX_VISITED = 64`. `Type13DeclarationLookupTest` (Phase 2) pins both hand-built-graph cases: a mutual cycle terminates with no declaration (mutation — removing the de-duplication guard — observed to hang; the guarded walk itself returns promptly), and a 70-node chain refuses while an 8-node chain resolves. |
| `TYPE-13-08` | **Cross-file receivers report honestly** | S | Full | Pinned by `Type13DeclarationLookupTest` (Phase 2) and wired into production by Phase 3: a `require`d un-annotated module's receiver resolves the member through the ordinary `LuaType` API and reports `sourceElement == null`. Phase 5 measured whether the declaration could be made to reach without changing a merge and found it cannot: `LuaTypeManagerImpl.getModuleType`'s AST fallback reads `getFileReturnType()`, which is `.write` alone (not `typeOf`'s write/read merge), so the exported `LuaType` already carries zero members before `fromLuaType`/`memberNodeFor` run — there is nothing for a mint-site fix to plumb. The assertion is left as-is; risks Gap 2.11 has the executed measurement. |
| `TYPE-13-09` | **Chained receivers** | C | Future Work | The second segment of `B:m1():m2()`. DR-01 measured only the chain's first segment (`m1` on `x`), where the structural route returns `LuaFile@0` and the nominal route carries the declaration; the second segment is unmeasured and `AGENTS.md` records that `visitFuncCall` models `nameAndArgsList.firstOrNull()` only. Phase 5 measured it: `visitFuncCall` seeds the whole `LuaFuncCall`'s value from the **first** segment's declared return and returns before visiting the second, so `getValueType` on `x:m1():m2()` reports `B` (the type of `x:m1()`) rather than `m2()`'s result — stronger than "no declaration," a silently wrong value. Risks Gap 2.12 has the executed measurement; no fix attempted, per plan. |
| `TYPE-13-10` | **No inference change** | M | Full | Every diagnostic the engine reports before, it reports after — message-for-message. Verified after Phase 3 lands: `LuaAnnotatedClassDiagnosticsTest` passes unchanged (3/3), and the `-PwithCorpus` ratchet reports no `Corpus regression:` line. |
| `TYPE-13-11` | **No cost regression** | M | Full | Landed Phase 4 (`implementation-plan.md`): `LuaTypeGraphRootResolutionBudgetTest` gained `conversionPathStaysWithinItsRootResolutionBudget`, which calls `graphTypeToLuaType` and reaches `LuaMemberDeclarations.declaringNodeOf` — measured `WRITE` = 574, `READ` = 648, budgeted 620 / 700. Case 15's prescribed mutation was executed and did **not** redden this test; risks Gap 2.10 records the executed finding and why it is a structural fact about this fixture (every node the walk could reach is already memo-warm from the ordinary type-check pass) rather than a headroom gap. |
...
| 15 | `TYPE-13-11` | the 80-call-site `annotatedCallSiteFixture()` already in `LuaTypeGraphRootResolutionBudgetTest` | build the snapshot, then convert the receiver's value type with `graphTypeToLuaType` | `RootAccessor.WRITE`/`READ` counts stay within the budgets Phase 4 measures and commits (574/648, budgeted 620/700) | have `declaringNodeOf` read `node.write` (instead of walking `upSet`) at each step → **executed in Phase 4, three variants, all reproduced `WRITE` = 574 exactly — did not redden.** `setName`'s node is itself the `declaresMember = true` node (BFS returns at step 0), and every node reachable from it via `upSet` is already `write`-memoized by the ordinary type-check pass before conversion runs, so a repeat read is free. Risks Gap 2.10 has the executed finding |
...
- [x] `TYPE-13-00-DR-01` and `TYPE-13-00-DR-05` have run and their results are recorded above.
- [x] Every `M` requirement has an executed test case with a named, reachable mutation — except
      `TYPE-13-10`, whose argument is recorded in "Test Cases".
- [x] `LuaAnnotatedClassDiagnosticsTest` passes unchanged (`TYPE-13-10`).
- [x] The corpus ratchet is run with `-PwithCorpus` and does not fail with `Corpus regression:`
      ...
      (`TYPE-13-10`, `TYPE-13-11`). Run at Phase 3 (24m49s, green); Phase 5 makes no production
      change, so it is not re-run — see the implementation plan's Phase 5 verification note.
- [x] `LuaTypeGraphRootResolutionBudgetTest` passes at its committed values, and its new
