...
- **Tasks**:
  - [x] Add `subtractMember` to `LuaTypeAlgebra` companion — design §3.3.
    - File: `src/main/kotlin/net/internetisalie/lunar/lang/psi/types/LuaTypeAlgebra.kt`
  - [x] Add `tryParseTypeofGuard` private function in `LuaTypesVisitor` companion — design §3.1.
  ...
  - [x] Modify `LuaTypesVisitor.visitIfStatement` — design §3.4.
    - Parse guards from condition expressions; inject narrowed scope bindings for each block.
  - [x] Create `TestFlowSensitiveType` test class.
...
- **Tasks**:
  - [x] Add `tryParseNilGuard` private function in `LuaTypesVisitor` companion — design §3.2.
  - [x] Ensure polarity model handles `~=` → "match branch gets original minus type" correctly.
  - [x] Extend tests for `~=` and `nil` guards.
...
- **Tasks**:
  - [x] In `visitIfStatement`, after processing each match branch, do NOT re-inject the original
  ...
  - [x] Extend tests for `elseif` chains.
...
- **Tasks**:
  - [x] Run full type-engine test suite: `./gradlew test --tests "*TypeEngine*"` — all existing
    tests must stay green.
  - [x] Add edge-case tests: unknown variable, `type()` with wrong arg count, nested `if` blocks,
    unreachable branch.
...
## Verification Tasks
- [x] Create `TestFlowSensitiveType` — covers all 13 TCs from `requirements.md`.
- [x] Verify no regressions: `./gradlew test` — existing type-engine tests (TestLuaTypeEnginePhase1, TestLuaTypeDomainModel, TestLuaTypeEngineSafety, etc.) must remain green.
- [x] Human verification: run `human-verification-checklists.md` scenarios in sandbox IDE.

## Task Summary

| Phase | Status | Priority |
|-------|--------|----------|
| Phase 1: `type()` guard recognition | completed | Must |
| Phase 2: `~=` and `nil` guards | completed | Must / Should |
| Phase 3: `elseif` chains | completed | Should / Could |
| Phase 4: Regression & hardening | completed | Must |