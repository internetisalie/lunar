...
| TARGET-08-07 | **Offline / failure handling** | M | **Partial** | A fetch with no network (or a hash/size mismatch, or an unsupported archive) surfaces an error balloon via the tools notification group and does not add the failing library to the effective (fetched) set. **Partial after Phase 3**: the fetcher detects failure, logs it, discards any partial tree and contributes no root; the *balloon* belongs to the caller (design §3.2, review N3) and lands with the Phase 5 settings UI. |
