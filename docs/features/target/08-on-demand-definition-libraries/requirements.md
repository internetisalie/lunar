...
| TARGET-08-04 | **Library-root registration** | M | **Full** | Each enabled + fetched library tree is exposed as a `SyntheticLibrary` source root via an `AdditionalLibraryRootsProvider`, so the indexer and LuaCATS parser see its `@meta` definitions. **TC 6 passes live as of 2026-08-04** — in GoLand with `busted` enabled, `assert.` completes luassert's API (`are`, `are_equal`, `are_same`, `is_true`, `add_formatter`, …) with signatures. Getting there took four fixes, none of them in this feature's own code: BUG-395 (member completion was file-local), BUG-398 (a `@class` named apart from its local materialized with no members; a stubbed module lost its exported type; the cross-file provider answered a member caret) and BUG-399 (a `@class` in a *library* file could not materialize at all, because type resolution was `projectScope`-bound). BUG-394 (bare-identifier path is projectScope-bound) remains open and is a separate caret — it governs completing `assert` itself, before the dot. |
