...
| TARGET-08-04 | **Library-root registration** | M | **Partial** | Each enabled + fetched library tree is exposed as a `SyntheticLibrary` source root via an `AdditionalLibraryRootsProvider`, so the indexer and LuaCATS parser see its `@meta` definitions. **Partial, not Full**: registration and *resolution* work — DR-03 asserts a project-file reference resolving into the library tree — but **TC 6 (member completion after `.`) does not pass**, so an acceptance criterion of this requirement is unmet. Cause is pre-existing and outside this feature: see BUG-395 (member path is file-local) and BUG-394 (bare-identifier path is projectScope-bound). |
