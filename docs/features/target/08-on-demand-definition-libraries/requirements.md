...
| TARGET-08-04 | **Library-root registration** | M | **Partial** | Each enabled + fetched library tree is exposed as a `SyntheticLibrary` source root via an `AdditionalLibraryRootsProvider`, so the indexer and LuaCATS parser see its `@meta` definitions. **Partial, not Full**: registration and *resolution* work — DR-03 asserts a project-file reference resolving into the library tree — and BUG-395 (member path is file-local) is **fixed as of 2026-08-04**, so a library global's own members now complete under unit test. Still Partial for two reasons: **TC 6 has not been re-run live** (its live gate is a VNC session), and BUG-398 means an inherited `---@class X : Y` member still does not complete, so `assert.` is thin rather than complete. BUG-394 (bare-identifier path is projectScope-bound) remains open and is a separate caret. |
