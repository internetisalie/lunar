...
| TARGET-08-03 | **On-demand fetch + cache** | M | **Full** | Enabling a library that is not yet cached downloads its tarball off-EDT, verifies size + SHA-256, and extracts it into a per-user cache keyed by library id + version. An already-cached library is reused with no network. Verification is **advisory, not fatal** — see DR finding #2: the commit SHA already pins the content, the tarball hash pins only the packaging, and nothing fetched is executed. |
| TARGET-08-04 | **Library-root registration** | M | **Partial** | Each enabled + fetched library tree is exposed as a `SyntheticLibrary` source root via an `AdditionalLibraryRootsProvider`, so the indexer and LuaCATS parser see its `@meta` definitions. **Partial, not Full**: registration and *resolution* work — DR-03 asserts a project-file reference resolving into the library tree — and both defects behind TC 6 are **fixed as of 2026-08-04** — BUG-395 (member path was file-local) and BUG-398 (a `@class` named apart from its local materialized with no members, a stubbed module lost its exported type, and the cross-file provider answered a member caret). Under unit test, `assert.` on the real busted/luassert shapes now completes the module's own **and** inherited members and nothing else. **TC 6 re-run live 2026-08-04 — still fails, on a new blocker (BUG-399).** The run did prove the member path works in a real IDE: `table.` completes `concat/insert/move/pack/remove/sort/unpack` where it previously completed nothing, and Ctrl+B on `assert` reaches busted's `assert = require("luassert")` in the library. It fails one step later — `require("luassert")` does not resolve into the definition-library root, so the global's type degrades to `Any` and `assert.` offers no members. BUG-394 (bare-identifier path is projectScope-bound) remains open and is a separate caret — it governs completing `assert` itself, before the dot. |
| TARGET-08-05 | **Index refresh on change** | M | **Full** | Changing the enable list refreshes the additional-library roots and drops resolve caches so newly-enabled definitions become resolvable without an IDE restart. |
| TARGET-08-06 | **Settings UI** | S | **Full** | A project `Configurable` lists the catalog with an enable checkbox per row, a fetched/not-fetched status column, and the license + attribution URL for enabled rows. |
| TARGET-08-07 | **Offline / failure handling** | M | **Full** | A fetch that fails (no network, an unsupported or truncated archive, or a `rootPrefix` matching nothing) surfaces an error balloon via the tools notification group and does not add the failing library to the effective (fetched) set. **A hash/size mismatch is no longer a trigger** — `ArtifactVerification.ADVISORY` warns and keeps the file, so it cannot fail a fetch (DR finding #2). Detection is the fetcher's (returns null, logs, discards any partial tree); reporting is the caller's, per design §3.2. |
| TARGET-08-08 | **Attribution surfaced** | S | **Full** | The license string and attribution URL of every library are shown read-only in the settings UI — for **every** row, not only enabled ones, so the obligation is visible before the user opts in. |
