...
| `EDITOR-01-05` | **Keyword block completion** | Completing `do`/`then`/`function`/`for`/`while` scaffolds the matching `end`; `repeat` scaffolds `until`. Coordinated with the Enter handler, not raw typed chars. | **S** | Not Implemented |
...
- Reuses the existing `LuaBraceMatcher` pair table; keyword-pair completion coordinates with the
  registered `EnterHandlerDelegate`s rather than firing on each keystroke.
