...
- **Tasks**:
  - [ ] Create `net.internetisalie.lunar.lang.editor.LuaMoveLeftRightHandler` (design §2.2)
  ...
  - [ ] Register `<lang.moveLeftRightHandler language="Lua" implementationClass="…LuaMoveLeftRightHandler"/>`
...
- **Tasks**:
  - [ ] Create `net.internetisalie.lunar.lang.editor.LuaStatementMover` (design §2.1) with
  ...
  - [ ] Register `<statementUpDownMover id="LuaStatementMover" implementation="…LuaStatementMover"/>`
        in `plugin.xml` (design §7).
...
- **Tasks**:
  - [ ] Implement `LuaStatementMover.blockBoundaryTarget` (design §3.1) computing the body-edge or
        delimiter-line `LineRange`, and set the `ENTERED_BLOCK` `MoveInfo` user-data flag.
  - [ ] Override `LuaStatementMover.afterMove` to call `CodeStyleManager.adjustLineIndent` on the
...
- **Tasks**:
  - [ ] Confirm `checkAvailable` returns `false` (not `prohibitMove()`) on the no-enclosing-statement
...
- [ ] `LuaMoveLeftRightHandlerTest` (extends `BasePlatformTestCase`) — covers TC-03a…TC-03d, plus a
      round-trip (right then left returns the original) mirroring `GroovyMoveLeftRightHandlerTest`.
- [ ] `LuaStatementMoverTest` (extends `BasePlatformTestCase`) — covers TC-01a…TC-01c, TC-02a/-02b,
...
|-------|--------|----------|
| Phase 1: Move Element Left/Right handler | todo | Should |
| Phase 2: Statement Up/Down — sibling move | todo | Must |
| Phase 3: Enter/leave blocks + re-indent | todo | Should |
| Phase 4: Line-move fallback | todo | Could |
