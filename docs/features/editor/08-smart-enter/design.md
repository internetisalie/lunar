...
### 2.1 The contract we require from EDITOR-01

A Kotlin object, **`net.internetisalie.lunar.lang.syntax.LuaKeywordPairs`**, exposing:

```kotlin
data class LuaBlockKeyword(
    val opener: IElementType,        // e.g. LuaTokenTypes.IF
    val closer: IElementType,        // e.g. LuaTokenTypes.END
    val separator: IElementType?,    // token that must sit between condition and body: THEN / DO / null
)

object LuaKeywordPairs {
    val blocks: List<LuaBlockKeyword>          // if/while/for/function/do → end, repeat → until
    fun byOpener(token: IElementType): LuaBlockKeyword?
}
```

Required rows (opener → closer, separator):
...
The table uses `LuaTokenTypes` constants (`lang/lexer/LuaTokenTypes.kt`), which are the token
element types the lexer emits (equal by identity to `LuaElementTypes.IF` etc.).

### 2.2 Fallback if EDITOR-01 has not landed

If `LuaKeywordPairs` does not yet exist when EDITOR-08 is implemented, the implementer creates it
in `lang/syntax/LuaKeywordPairs.kt` **with exactly the shape and rows above** (this is the minimal
surface EDITOR-01 also needs). See `risks-and-gaps.md` DR-01 for the ordering handshake. Either
way the file lives at `lang/syntax/LuaKeywordPairs.kt` and is owned jointly. The fixers depend
only on the six rows and the two accessors — never on EDITOR-01 internals.
...
Algorithm:
1. `val pair = keywordPairFor(element) ?: return` — maps the PSI element type to a
   `LuaBlockKeyword` row: `LuaIfStatement`/`LuaFuncDecl`/`LuaLocalFuncDecl`/`LuaWhileStatement`/
   `LuaNumericForStatement`/`LuaGenericForStatement`/`LuaDoStatement` → the matching opener row;
   `LuaRepeatStatement` → the `REPEAT`/`UNTIL` row. Return early for any other element.
2. `if (LuaSmartEnterUtil.hasCloser(element, pair.closer)) return` — a closer already present as a
...
4. For `REPEAT`: `document.insertString(insertAt, "\nuntil ")` and
...
1. Map `element` → `LuaBlockKeyword` as above; `val sep = pair.separator ?: return` (function/do/
   repeat have no separator → nothing to do).
...
object LuaSmartEnterUtil {
    fun keywordPairFor(element: PsiElement): LuaBlockKeyword?     // PSI type → LuaKeywordPairs row
...
- `keywordPairFor` maps by PSI class (`when (element) { is LuaIfStatement -> …byOpener(IF) }`),
  using `LuaKeywordPairs.byOpener`.
