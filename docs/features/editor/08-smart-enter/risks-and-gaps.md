...
| :--- | :--- | :--- | :---: | :--- |
| R-01 | **Partial skeleton parses to `ERROR_ELEMENT`s, not a clean `LuaIfStatement`.** `if x<caret>` has no `then`/`end`, so the parser may not build a `LuaIfStatement` node at all — `getStatementAtCaret`/`openerKeywordFor` could miss it. | Fixers no-op; feature silently does nothing. | High | Design §3.4 keys off **direct child tokens** (`hasChildToken`) and the leaf under the caret, not on a well-formed subtree. If PSI-class matching proves unreliable for a bare opener, fall back to matching the opener **keyword token** at the statement start and looking it up directly in `LuaBlockPairs.separatorByOpenerKeyword`/`.terminatorByOpenerKeyword`. De-risk with DR-03. |
...
| :--- | :--- | :--- | :--- |
| `EDITOR-08-00-DR-01` | **Confirm the `LuaBlockPairs` extension is additive.** Verify the two new opener-keyword maps (design §2.1) do not disturb the existing `terminatorByOpener`/`insertTextFor`/`terminatorForOwner` used by the COMP-08 Enter handler and EDITOR-01's `LuaKeywordBlockCloser`. | Before Phase 0 coding. | Full suite green after adding the maps; no change to Enter-handler / EDITOR-01 behavior. No parallel `LuaKeywordPairs` table exists. |
