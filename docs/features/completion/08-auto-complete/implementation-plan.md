...
- **Tasks**:
  - [x] Create `net.internetisalie.lunar.lang.syntax.LuaBlockPairs` object with `terminatorByOpener`
        and `insertTextFor` maps — realizes design §2.3.
  - [x] ~~Refactor `LuaPairedBraceMatcher.getPairs()` to derive its keyword `BracePair`s from
        `LuaBlockPairs.terminatorByOpener`~~ — **intentionally NOT done.** design §2.3 (authoritative,
        post-review) forbids it: the matcher keys its `if` pair on `IF`→`END` (it highlights the
        `if`↔`end` span) while `terminatorByOpener` keys on `THEN`→`END`; deriving the matcher from that
        map would re-key brace matching and break `if`↔`end` highlighting. The matcher keeps its own
        `BracePair` list; `LuaBlockPairs` is the single source only for the Enter handler's maps.
  - [x] Edit `LuaEnterHandler.preprocessEnter` to add the balance check: resolve the owning
  ...
  - [x] Add the STATELESS `postProcessEnter` reformat/caret step (no instance field; re-derive the
        just-inserted terminator from the committed document) on `LuaEnterHandler` using
        `CodeStyleManager.adjustLineIndent` — realizes design §3.5.
...
- **Tasks**:
  - [x] Extend `LuaEnterHandler` to look up ANY opener in `LuaBlockPairs.terminatorByOpener` (drop the
...
- **Tasks**:
  - [x] Create `net.internetisalie.lunar.lang.completion.LuaEnterBetweenBlockHandler :
        EnterHandlerDelegateAdapter` implementing the §3.4 algorithm — realizes design §3.4.
  - [x] Register `<enterHandlerDelegate>` for it in `plugin.xml` adjacent to `LuaEnterHandler` —
        realizes design §7.
...
## Verification Tasks
- [x] Extend `src/test/kotlin/.../lang/completion/LuaEnterHandlerTest.kt` with the explicit
...
- [x] Add `testEnterAfterTableBrace` (`local t = {<caret>` ⇒ `{`…`}`) — covers TC 3 (COMP-08-03).
- [x] Add coverage for `while…do`, numeric `for…do`, generic `for…do`, bare `do`, `repeat…until` openers
      — covers COMP-08-03.
- [x] Add `testEnterBetweenMatchedFunction` (caret between `function f()` and its `end`) — covers TC 4
      (COMP-08-04).
- [x] Add a reformat/caret assertion (terminator line and body line correctly indented) — covers
      COMP-08-05.
- [x] Regression: confirm `LuaPairedBraceMatcher` and the DOC `lang.format.LuaEnterHandlerDelegate`
...
|-------|--------|----------|
| Phase 1: Balance check + shared pair map + reformat | done | Must |
| Phase 2: Full opener coverage incl. table `{`→`}` | done | Should |
| Phase 3: Between-pair smart indent | done | Should |
