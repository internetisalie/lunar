...
| COMP-09-00-DR-16 | Re-measure the `catsClassTags` / `LuaImplicitFields:76` / `LuaTypesVisitor:1349` walks with medians of ≥5, against the **right door** for each | COMP-09-02 scope (design §4.11) | **DONE 2026-08-12 — design §1.11.5. The owed per-site right-door half is now measured, and the scope decision is unchanged.** Anchors re-grepped first, and three moved or were miscounted: `catsClassTags` is `LuaTypeManagerImpl:435` called at `:396` (not `:436`/`:393`), and *"`catsClassTags`" and "`LuaTypeManagerImpl:436`" are the same site named twice* — there are **three** remaining sites, not four. `LuaImplicitFields:76` and `LuaTypesVisitor:1349` are exact. `LuaMemberFieldNavigation:32` is exact and **out of scope** (a navigation site, excluded by name in requirements.md). Measured warm, medians of five: **`catsClassTags` 10 970 µs**, **`LuaImplicitFields.collect` 17 844 µs** over 3 400 fields — both on the `@class` door, together **29 ms** against that door's **269 ms** cold and **103 ms** warm-file marginal. So they are **not** why that door misses budget (DR-29) and converting them would not bring it inside; §4.3/§4.6 rightly do not touch them. `LuaTypesVisitor:1349` (`seedAmbientGlobals`) is **under budget by construction** and DR-26 measured why: it reads only files *named* `global.lua`, none exists on the default target, and each of the five that do (`redis-5/6/7`, `valkey-7.2/8`) is **7 lines**. The Phase 0 half below stands as the record. ⚠ Note the Phase 0 "29 ms" was a *shaped candidate*, not the site: the real `catsClassTags` is 11 ms. **Superseded detail (Phase 0):** — the medians half is settled and it makes the old scope-out look worse, not better: `CompNineSection32Test` candidate B (the `catsClassTags`-shaped walk) is **29 ms median of 5** (24/25/29/46/52) against a **41 ms** warm-file `@class` door, i.e. roughly *two thirds* of that door's marginal cost, not the 2 % the retracted "22 ms of 949 ms" implied. Candidate C (`getAllKeys` 16 180 keys + `getElements`) is **18 ms**. Still owed: the per-site right-door analysis — `LuaTypesVisitor:1349` is on the `resolveGlobal` door and neither figure applies to it. The scope decision does not change: §4.3/§4.6 do not touch these sites |
...
| COMP-09-00-DR-23 | Rule S's residual: a consumer file that **binds a name which is also a project-wide global** still pays the O(file) walk. 8–16 ms at 4 002 lines. Decide whether the bound-name set needs caching, and if so what invalidates it | design §4.14 cost note; Phase 5 | **done 2026-08-12 — design §1.11.2. DECIDED: it stays as written, with the figure recorded.** Re-measured on a 4 002-line consumer, medians of five: **560 µs** bound-at-top (the walk exits on the binding), **14 692 µs** bound-at-bottom, **10 880 µs** unbound (the full walk, the true worst case — no early exit), against `forFile` on the *same* file at **44 115 µs** *(single — unrepeatable by construction)*. A second run gave 602 / 16 684 / 10 304 / 46 313 µs. The prototype's 8–16 ms is **confirmed**. The cached bound-name set is rejected on a **mechanism**, not a cost judgement: Rule S reads `receiverExpr.containingFile`, which during completion is the per-session **copy** (the call site says so; BL-8 is the defect that established it), so a `CachedValuesManager` cache on the copy can never hit, and one keyed on `originalFile` with a PSI-modification dependency invalidates on every keystroke in exactly the file DR-23 is about. Also bounded above by the path it precedes (10.9–14.7 ms vs 44 ms), and already inside door 1's 12.2 ms end-to-end cold median, which is a completion that ran it |
| COMP-09-00-DR-24 | §4.12's withdrawal does not cover one case: a file whose **only** receivers are opaque still pays one snapshot build, because nothing hoists it. Measure it | design §4.12 residual; Phase 5 | **done 2026-08-12 — design §1.11.3. Measured, and inside budget on the quotable quantity.** Five distinct opaque receivers in five files with **no tier-1 completion anywhere before them** — which is what the withdrawal rests on, so the measurement had to remove it. Medians of five: **43 736 µs**, and **36 695 µs** on a second run. The in-order print settles what a sorted one could not: **the worst sample is the first**, because the five receivers share one `luassert.lua` whose snapshot the first completion builds — *(single — unrepeatable by construction)*, at 99 546 µs one run and 260 319 µs the other. Recorded as §4.12's stated watch item, **not** filed as a defect: the NFR is asserted on the median and the median is comfortably inside, and the outlier is the library graph build this feature routes around for tier 1 and never claimed to remove |
| COMP-09-00-DR-25 | `LuaReceiverMemberIndex.Indexer.map` makes **four separate `findChildrenOfType` passes** over the same PSI (§4.3's five sources) and is the most expensive of the three whole-project Lua indexers. Decide whether one shared traversal is worth it | design §4.8a; Phase 5 | **DEFERRED with an owner — filed as BUG-437 (roadmap, Wave 20). DECIDED: worth doing, not here.** Design §1.11.4. Three corrections to this row's own premises, each from a run. **(1) The count is wrong**: `map` has **five** traversal call sites over three element types, and **three of them are the identical `LuaAssignmentStatement` walk** (`indexMemberAssignments`, and `indexTableLiteralFields`/`indexOpaqueBindings` via `forEachBareBinding`). **(2) DR-18's number was not re-usable and the naive re-measure was wrong in the other direction**: timing `map` first gave 256 ms, but reordering showed **the cost follows the measurement position, not the function** — the first `findChildrenOfType` caller pays the file's one-time AST expansion (~200–280 ms), which `FileContentImpl` defers past `getPsiFile` to first tree access. Timed after the expansion the three indexers are **67 / 20 / 6 ms**, reproducing DR-18's warm 61 / 20 / 6 — so **DR-18 stands, confirmed rather than superseded**, and its ranking holds. **(3) The widened filter changed the file *set*, not the per-file cost**: counted on disk, this repo adds **zero** files (252 `.lua`, no `.rockspec`/`.luacheckrc`/`.busted`) and the out-of-repo `test/` corpus adds **177** to 1 071 (+16.5 %), mean 2.1 KiB. **The decision**: one `processElements` walk costs the same as one `findChildrenOfType` (10 016 µs vs 10 767), so four of the five walks are redundant at ~10 ms each — about **40 ms of the index's 67 ms per file**. Worth ~60 % of the per-file build cost, but one-off, persisted, on no latency path, and it edits a shipped index (content divergence ⇒ version bump + corpus gate). Owner: **BUG-437** |
| COMP-09-00-DR-29 | **The `@class` door misses NFR-1 and no prior figure said so.** Task 1 asked for both doors against the 100 ms budget; door 2 had never had a cold medians-of-five figure at all | design §1.11.1; NFR-1; Phase 5 | **DEFERRED with an owner — filed as BUG-438 (roadmap, Wave 20).** Five distinct wide `@class` receivers in five files: `resolveType` + `materialize` + `getMembers` is **269 459 µs** median (257 149 µs on a second run), and the same door end to end through `completeBasic()` (`---@type Klass$i` on a local, then `v.`) is **322 692 µs** — **3× the budget**, at 3 600 members. **Not a regression and no improvement claimed**: no comparable prior figure exists, and this is the residual §1.6 named in advance ("an index of names serves completion; the checker still needs types and therefore still pays"). Phase 3 converted this door for the **work bound** (COMP-09-09, green) and never claimed its latency; COMP-09-08 is and always was asserted on the completion door. §1.11.5 rules out the two remaining COMP-09-02 sites as the cause — together they are 29 ms of the 269 ms. Owner: **BUG-438** |
...
| COMP-09-00-DR-07 | §3.3: would narrowing cache invalidation beat indexing? Previously only a "TBD" under Technical Debt with no task — the DoD requires every open question be tracked | design §3.3 | **done — DECIDED, design §1.2a; re-confirmed on Phase 5's numbers, design §1.11.7, and NOTHING remains.** The post-edit case is shipped (TYPE-11's `churnDependencyFor`); the cold completion door is **12.2 ms**, and no invalidation strategy improves on a path that never builds the snapshot; and the one door still over budget (DR-29) misses on a **first build** — the declaring file's parse plus per-member type construction — which narrowing, whose whole subject is *keeping* a snapshot across unrelated edits, cannot make cheaper. Original decision text: **DECIDED, design §1.2a.** Narrowing takes the post-edit case from ~200 ms to ~1 ms but cannot touch cold — hundreds of ms (392 / 463 / 1 152 across three runs), every session's first completion, over budget under all three — or the member-count scaling. The decision rests on the **direction only**: per DR-08 no ratio between two of this harness's figures is quotable, and the earlier wording here quoted three. Complement, not alternative: index now, narrow separately |
...
- ~~**TBD: the four caches.**~~ **RESOLVED 2026-08-12 (Phase 5, design §1.11.6) — all four are
  re-measured and all four STAY.** `typeCache` saves 103–477 ms per hit (hit costs 9 µs);
  `globalCache` saves 631 ms (hit 9 µs); `LuaTypesSnapshot.forFile`'s per-file cache saves 44 ms on a
  4 002-line consumer and 844–955 ms on the 123 KiB library (hit 13 µs); `GlobalSymbolRankingService`'s
  two `getAllKeys` caches save ~1.3 ms per completion invocation and answer a *ranking* question the
  receiver index cannot. `moduleCache` is not separately measured — same structure, same door — and
  that is stated rather than implied. Removing a cache remains a separate, measurable change, and
