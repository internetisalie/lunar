...
| COMP-09-00-DR-02c | Per-keystroke or per-session? | severity | **done, and it overturned §1.2** — `CompNineDr13Test`, medians of 5, time-to-**first**. Cold **1 152 ms**, warm **1.0 ms**, after one unrelated keystroke **264 ms**. So 22 % is repaid, not the 76 % §1.2 claimed, and the per-keystroke/per-session dichotomy is false: neither, but 2.6x over budget either way |
...
| COMP-09-00-DR-16 | Re-measure the `catsClassTags` / `LuaImplicitFields:76` / `LuaTypesVisitor:1349` walks with medians of ≥5, against the **right door** for each | COMP-09-02 scope (design §4.11) | todo — the "22 ms of 949 ms" scope-out used a denominator §1.8 disavows (383 ms re-measured), a single-shot numerator, and a `@class`-door figure for a site on the `resolveGlobal` door. They stay out of scope on *irrelevance to the critical path*, not on cost; this settles the cost |
| COMP-09-00-DR-17 | Derive COMP-09-08 assertion 2's independence factor rather than picking one | design §4.10a, COMP-09-08 | todo — rule stated: `ceil(p95 / p50)` over ≥5 cold samples of the **narrow** receiver alone, i.e. the harness's own noise floor measured where member count cannot be the variable |
...
| COMP-09-00-DR-07 | §3.3: would narrowing cache invalidation beat indexing? Previously only a "TBD" under Technical Debt with no task — the DoD requires every open question be tracked | design §3.3 | **done — DECIDED, design §1.2a.** Narrowing takes the post-edit case 264 ms -> 1.0 ms but cannot touch the cold 1 152 ms (every session's first completion) or the 40x member-count scaling. Complement, not alternative: index now, narrow separately |
