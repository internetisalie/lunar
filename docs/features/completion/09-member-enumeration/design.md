...
| 4 | `filesVisited ==` the number of **candidate** files the selection rule chose, and is unmoved by unrelated indexed content | `globalMembership` (§4.5c) | the completion door reads only its candidates; more means the selection rule leaked |

*(**Amended by BUG-439, 2026-08-15.** The parenthetical "(1 for a receiver bare-bound in one file in the chosen scope)" was true of the fixture and of the rule, and the rule was wrong: candidates came from `LuaGlobalAssignmentIndex` alone, which lists files that assign the **global**, so a sibling file that merely *extends* it was never a candidate and its members were never offered. On `LuaCATS/love2d` that cost `love.` all 19 of its submodules. `candidates` now has a second tier — this index's own key space, minus files where a `LOCAL_BINDING` sentinel says the receiver is a file-local — and `membershipOver` unions the candidates instead of taking the first. The count is therefore the receiver's declaring-file count, and the assertion carries an invariance clause, because a fixture-specific constant cannot distinguish a key-space scan from a correct lookup.)*
