...
- **Tasks**:
  - [ ] Add `<template>` blocks `if` / `ifel` / `lfun` / `while` to `lua.xml` per design §2.2.2
  ...
  - [ ] Confirm each carries `description`, `toReformat="true"`, `toShortenFQNames="true"` and
...
- **Tasks**:
  - [ ] Create `net.internetisalie.lunar.lang.completion.templates.LuaCodeContextType` — realizes
  ...
  - [ ] Implement `isInContext` — realizes design §3.1: companion `SUPPRESS` `TokenSet` =
  ...
  - [ ] Register `<liveTemplateContext implementation="…LuaCodeContextType"/>` in `plugin.xml` —
    realizes design §7 (keep the existing `LuaTemplateContextType` registration).
  - [ ] Migrate **all** bundled templates' context option from `LUA` to `LUA_CODE` in `lua.xml`
...
- **Tasks**:
  - [ ] Add `repeat` / `forip` / `req` / `mod` `<template>` blocks to `lua.xml`, each bound to
...
- **Tasks**:
  - [ ] Create `LuaSurroundContextType` — realizes design §2.3: id `"LUA_SURROUND"`, name
  ...
  - [ ] Register `<liveTemplateContext implementation="…LuaSurroundContextType"/>` in `plugin.xml` —
    realizes design §7.
  - [ ] Add surround `<template>` blocks `surr_if` / `surr_for` / `surr_do` / `surr_fn` to `lua.xml`
...
## Verification Tasks
- [ ] Extend `LuaLiveTemplateTest` to assert all new abbreviations load in group `Lua` — covers TC 1–5
  (loaded-set portion).
- [ ] Add a behavioural test expanding `if`/`while` in real code (TC 2, 5) via the fixture.
- [ ] Add a suppression test: caret inside `"…"` and inside `-- …`, assert no Lua template offered —
  covers TC 6 (the COMP-07-10 defect fix).
- [ ] Manual: Surround a selection with `if`/`for`/`do`/`function` via Ctrl+Alt+T (COMP-07-11).
- [ ] Run `python3 scripts/lint_planning.py` and
...
|-------|--------|----------|
| Phase 1: Must insertion templates | planned | Must |
| Phase 2: LuaCodeContextType + migrate | planned | Should |
| Phase 3: Should insertion templates | planned | Should |
| Phase 4: Surround templates | planned | Should |
