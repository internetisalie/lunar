...
- **Member Field Navigation (NAV-12)**: `Go to Declaration` on a dotted field access (`package.path`, `obj.field`) now navigates to the `receiver.field = value` declaration, via a new qualified-name member-field index (`LuaMemberFieldIndex`). Field assignments aren't stubbed, so they were previously unreachable; keying by the full `receiver.field` keeps them from colliding with unrelated same-short-name module symbols. Completes `NAV-01-03` (Table Fields → Full). Field *quick documentation* rendering is the remaining piece.
